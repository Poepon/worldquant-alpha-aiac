...
**诚实限定(纪律)**:① N=12 统计弱 → 稳健结论是「**未碰不比已用差**」(关键否定结果),「未碰更好」n=12 不能强主张;② **绝对值都低**(p50 0.3-0.43)→ regime DOWN 下两组都不可提交,canary 验的是**信号存在性(相对)非可提交性(绝对)**;③ 测简单模板,非字段全潜力。可选 N≈30 确认性 canary 把统计做实。
