...
| 2026-05-18 | v1.47 | **Phase 3 R1b.2b ship — wire hypothesis_mutate node in workflow + 2 smoke tests**(R1b.2 progress 2/3 PRs;R1b plan v1.3 §4.2 + workflow integration)。**Scope**:`workflow.py` import `node_hypothesis_mutate` alongside `node_code_gen_retry` + register `'hypothesis_mutate'` node via `partial(...llm_service=)` + replace R1b.1c placeholder route `'hypothesis_mutate' → 'save_results'` with real node + `add_edge('hypothesis_mutate', 'save_results')` for cycle exit。**Per plan §4.2** mutate does NOT loop back to validate within the graph — new hypothesis needs fresh `rag_query → hypothesis_propose → code_gen` round which only OUTER mining loop(mining_tasks.py)drives。Mutate sets `state.r1b_pending_new_hypothesis` + drops to save_results;next outer iteration consumes pending hypothesis。Cross-round persistence to MiningTask.config JSONB 推 R1b.2c integration。**2 new workflow smoke tests** added(ENABLE_R1B_HYPOTHESIS_MUTATE=True alone wires mutate / both R1b flags ON wires both nodes — no graph build error)。**196 PASS + 2 SKIPPED cumulative**(194 + 2 new smoke)。**R1b.2 progress 2/3 PRs**(R1b.2a `798e055` node + prompt + R1b.2b `31417bf` workflow wire);剩 R1b.2c integration test exercising full mutate cycle + cross-round persistence to MiningTask.config + outer mining_tasks loop wires pending_new_hypothesis read on next-round hypothesis_propose + 7d GO gate observation。**R1b 5-sub-phase progress 1/5 SHIP-COMPLETE + R1b.2 (2/3)**。**Backward-compat**:both R1b flags default OFF → no R1b node registered → legacy linear `add_edge('evaluate', 'save_results')` byte-equivalent。Flag flip now activates entire R1b cycle(retry + mutate)in one step。**Phase 2 5/5 + Phase 3 6/8 + R8-v2 3/6 + flat-F4 Step 1 + Q10 12/12 COMPLETE + R1b.1 COMPLETE + R1b.2 (2/3) shipped = master plan v1.47 路线 ~99.6% 推进** |
| 2026-05-18 | v1.46 | **Phase 3 R1b.2a ship — hypothesis_mutate node + prompt + 15 tests (R1b.2 sub-phase kickoff)**(R1b.2 progress 1/3 PRs;R1b plan v1.3 §4.2 + §4.3 dataset-cycle-scoped mutation)。**Scope**:`agents/prompts/r1b_mutate.py` NEW — `R1B_MUTATE_SYSTEM` + `R1B_MUTATE_USER_TEMPLATE` + `build_r1b_mutate_prompt(original_hypothesis, original_alpha_outcomes, r5_c1_reason, failure_tree_summary, region, dataset_id, pillar)` per plan [V1.1-A1-3] 嵌入 failure_tree summary 防止 LLM 复proposing 已失败 hypotheses;[V1.2-A2-2] R5 c1 reason 是 LLM-generated hypothesis vs description alignment diagnosis(1-step CoSTEER reflection)。defensive defaults render placeholders。**`agents/graph/nodes/r1b_loop.py`** +`node_hypothesis_mutate`(+~220 LoC):**plan §4.2 dataset-cycle-scoped**(not per-alpha)— groups FAIL+(hypothesis|both) alphas by hypothesis statement → picks highest-impact group(most failed alphas)→ ONE LLM call。Emits `pending_new_hypothesis = {statement, rationale, expected_signal, key_fields, suggested_operators, parent_hypothesis_statement, diff_from_original}` for next-iteration consumption。Self-guards(V-26.57):per-cycle mutation budget(`R1B_MAX_MUTATIONS_PER_DATASET_CYCLE=2`)+ shared token cost ceiling。Per-hypothesis try/except — 单 LLM failure 不阻塞 round。`_make_mutate_log_row` writes mutate_hyp rows to dedicated `r1b_retry_log`。**15/15 PR2a tests PASS**:2 prompt(renders + defensive defaults)+ 5 trigger gating(hypothesis / both / skip impl / skip unknown / skip non-FAIL)+ 2 dataset-cycle dedupe(same hypothesis 1 LLM call / picks highest-impact across mixed hypotheses)+ 2 budget guards(per-cycle exhausted / token ceiling)+ 4 soft-fall + payload(LLM raise bumps counter / same-statement no emit / empty-statement no emit / full payload structure verified)。**194 PASS + 2 SKIPPED cumulative**(Q10 140 + R1b 54 = retry 14 + routers 16 + integration 9 + **mutate 15**)。**R1b.2 progress 1/3 PRs**(R1b.2a node + prompt + tests);剩 R1b.2b workflow wire(replace placeholder 'hypothesis_mutate' → 'save_results' in workflow.py with real node + edge to hypothesis-propose for next-iteration consumption of pending_new_hypothesis)+ R1b.2c integration tests + 7d GO gate observation。**R1b 5-sub-phase progress 1/5 SHIP-COMPLETE + R1b.2 (1/3)**。**Backward-compat**:`ENABLE_R1B_HYPOTHESIS_MUTATE` default OFF → node 永不 invoked,workflow.py 还 wire placeholder for it。Pure additive。**Phase 2 5/5 + Phase 3 6/8 + R8-v2 3/6 + flat-F4 Step 1 + Q10 12/12 COMPLETE + R1b.1 COMPLETE + R1b.2a kickoff = master plan v1.46 路线 ~99.5% 推进** |
| 2026-05-18 | v1.45 | **🎉 Phase 3 R1b.1 SUB-PHASE SHIP-COMPLETE — PR1d integration tests + flag-OFF byte-equiv guard + 9 tests (4/4 R1b.1 PRs done)**(R1b plan v1.3 §3.6 final coverage + §3.7 GO-gate-observation ready)。**Scope**:`backend/tests/integration/test_r1b_loop_integration.py` NEW 9 tests closing plan §3.6:**3 log-row write verification**(full payload — task_id / round_idx / triggering_attribution / triggering_attribution_source='r5_judge' when c2_reason populated / sha256[:64] hash / new_expression / changes_made / outcome='pending' / loop_error None / llm_cost > 0 / 64-char hash / LLM-raise records 'LLM provider 500' in loop_error / same-expression no-op records 'same/empty expression' explanatory error)+ **2 flag-OFF byte-equivalence regression**(plan §3.6 #1 the single most important R1b test,mirror of Q10):sentinel via sys.modules tracking — assert `backend.agents.graph.nodes.r1b_loop` NOT in sys.modules after workflow build with both R1b flags False;flag ON r1b_loop IS imported)+ **2 state reset boundary**(plan §3.5 [V1.1-A1-2]):all 6 R1b MiningState fields default 0/[]/None/0.0 + serialization round-trip preserves)+ **2 one-cycle integration smoke**(full retry replaces alpha + counters bump + validation state reset + retry chain captured / multi-alpha partial retry 3 alphas different attributions only rewrites implementation one,leaves hypothesis + unknown untouched)。**179 PASS + 2 SKIPPED cumulative**(Q10 140 + R1b 39 = retry 14 + routers 16 + integration 9)。**R1b.1 SUB-PHASE COMPLETE 4/4 PRs**:(1) `9f506b9` foundational(flags + R1bRetryLog + Alembic R1b-A)+(2) `106fea6` retry node + prompt + 14 tests +(3) `3cfa3c2` workflow edges + 2 routers + 16 tests +(4) **`eb26bd6` integration tests + byte-equiv guard + 9 tests**。**R1b.1 production deploy sequence**:(1) `/ops/feature-flags PATCH ENABLE_R1A_HOOK=true`(R1a attribution source)(2) `/ops/feature-flags PATCH ENABLE_LLM_JUDGE=true`(R5 typed attribution;无 91% UNKNOWN nullifies R1b)(3) `/ops/feature-flags PATCH ENABLE_R1B_RETRY_LOOP=true`(4) Observe ≥7d / ≥50 retries / ≥15% pass rate via `r1b_retry_log`(5) R1b.2 ship hypothesis_mutate sub-phase after R1b.1 GO。**Backward-compat preserved**:2 PR1d tests explicitly assert no R1b code path taken when flags OFF — sentinel via sys.modules tracking。Zero risk to existing production。**R1b 5-sub-phase progress 1/5 SHIP-COMPLETE**(R1b.1 ✓);剩 R1b.2 hypothesis_mutate(5d)+ R1b.3 failure trees(4d)+ R1b.4 typed pipeline(5.5d)+ R1b.5 DAG retry reward(3d)= 17.5d remaining。**Phase 2 5/5 + Phase 3 6/8 + R8-v2 3/6 + flat-F4 Step 1 + Q10 12/12 COMPLETE + R1b.1 SUB-PHASE COMPLETE = master plan v1.45 路线 ~99.5% 推进**(实际 ship + 17.5d R1b 2-5 multi-session + phase15-D + flat-F4 Step 2-5 operator decision) |
| 2026-05-18 | v1.44 | **Phase 3 R1b.1c ship — workflow edges + 2 routers + 16 tests (R1b cycle LIVE behind flag)**(R1b.1 progress 3/4 PRs;R1b plan v1.3 §2.1 + §2.3 + §2.4 core integration)。**Scope**:**`agents/graph/edges.py`** +2 routers per plan §2.3-§2.4 — `_route_after_evaluate(state)` gates entering retry sub-graph(flag OFF → save_results;flag ON + actionable attribution → r1b_retry_router;UNKNOWN never triggers per graceful handling)+ `_route_after_r1b_retry(state)` 3-way fork(`code_gen_retry` / `hypothesis_mutate` / `save_results`)with 3 redundant budget guards(per-alpha retries / per-cycle mutations / token cost ceiling)— BOTH attribution → mutate dominates retry per [V1.0-A2-3](mutate change family entirely makes retry stale);any budget fails → save_results。**`agents/graph/workflow.py`** flag-gated wiring inside `_build_graph`:`ENABLE_R1B_RETRY_LOOP=True OR ENABLE_R1B_HYPOTHESIS_MUTATE=True` activates cycle — registers `code_gen_retry` + `r1b_retry_router` nodes + 2 conditional edges + `add_edge('code_gen_retry', 'validate')` for cycle closure validate→simulate→evaluate→retry。With both flags OFF,falls through to legacy `add_edge('evaluate', 'save_results')` — byte-equivalent。`hypothesis_mutate` path routed to `save_results` 暂(R1b.2 wires real mutate node)— keeps graph build legal even if operator flips mutate flag prematurely。Loop termination per [V1.1-A1-1]:budget guards in router AND node V-26.57 redundancy。**16/16 PR1c tests PASS**:6 `_route_after_evaluate`(flags off save / retry on impl → router / retry on hyp skip / mutate on hyp → router / unknown skipped / non-FAIL skipped)+ 8 `_route_after_r1b_retry`(code_gen_retry / hypothesis_mutate / BOTH mutate dominates / budget exhausted save / token ceiling save / mutations exhausted falls to retry / no FAIL save / unknown save)+ 2 workflow.py smoke(flag OFF + flag ON both build cleanly)。**170 PASS + 2 SKIPPED cumulative**(Q10 140 + R1b 30 = retry 14 + routers 16)。**R1b.1 ALGORITHM CHAIN COMPLETE** — only PR1d(additional integration tests + GO gate observation post production-flip)remaining for R1b.1 ship。**R1b.1 progress 3/4 PRs**(R1b.1a `9f506b9` foundational + R1b.1b `106fea6` retry node + prompt + R1b.1c `3cfa3c2` routers + wire);剩 R1b.1d + R1b.2/3/4/5 sub-phases。**Backward-compat**:both flags default OFF → workflow builds legacy linear path → zero production behavior change until operator flips。**Phase 2 5/5 + Phase 3 6/8 + R8-v2 3/6 + flat-F4 Step 1 + Q10 12/12 COMPLETE + R1b.1 (3/4) shipped with cycle LIVE = master plan v1.44 路线 ~99% 推进** |
| 2026-05-18 | v1.43 | **Phase 3 R1b.1b ship — retry node + prompts + MiningState fields + 14 tests**(R1b.1 progress 2/4 PRs;R1b plan v1.3 §3.2 + §3.4 + §5.1)。**Scope**:**`state.py`** +6 R1b fields all Optional / default-safe per phase15-C(r1b_retries_attempted_this_alpha + r1b_mutations_attempted_this_cycle R1b.2 placeholder + r1b_token_cost_this_alpha + r1b_loop_attribution_evidence + r1b_mutated_hypothesis_ids + r1b_pending_new_hypothesis);reset per LangGraph invocation,cross-round persistence 推 R1b.1c persistence node。**`agents/prompts/r1b_retry.py`** NEW — `R1B_RETRY_SYSTEM` + `R1B_RETRY_USER_TEMPLATE` + `build_r1b_retry_prompt(original_expression, original_hypothesis, failure_metrics, r1a_evidence, r5_c2_reason, allowed_fields)` per plan [V1.2-A2-2] 嵌入 R5 c2 alignment reason 作 1-step CoSTEER reflection;defensive defaults render placeholders。**`agents/graph/nodes/r1b_loop.py`** NEW(~280 LoC)— `node_code_gen_retry` + `_estimate_cost`(mirror R5 30/70 rate split with COST_PER_1K_INPUT/OUTPUT for haiku-4-5 / opus-4-7 / deepseek-chat / gpt-4)+ `_write_r1b_retry_log_rows`(dedicated AsyncSessionLocal + soft-fail per plan §6.4)。**Self-guards (V-26.57)**:per-alpha retry budget + token cost ceiling 两 early-return 不调 LLM if exhausted。**Per-alpha try/except** — 单 LLM failure 不阻塞 round。`_r1b_retry_chain` 累积 metrics via V-26.79 rebind。Preserves `original_expression` on first retry。Resets is_valid/is_simulated/quality_status 使 alpha re-flow validate → simulate。**14/14 PR1b tests PASS**:2 prompt(renders all sections + defensive defaults)+ 4 trigger gating(triggers on implementation / skips unknown / skips quality!=FAIL / handles BOTH)+ 2 budget guard(retry exhausted early-return / token ceiling early-return)+ 3 soft-fail(LLM exception bumps counter + same-expression no-op + empty no-op)+ 3 _estimate_cost(zero / haiku rate / unknown-model default)。**154 PASS + 2 SKIPPED cumulative**(Q10 140 + R1b 14)。**R1b.1 progress 2/4 PRs**(R1b.1a `9f506b9` foundational + R1b.1b `106fea6` retry node + prompt + 14 tests);剩 R1b.1c workflow edges + 2 routers(`_route_after_evaluate` + `_route_after_r1b_retry` per plan §2.3-§2.4)+ R1b.1d 8 more tests + 7d GO gate observation。**Backward-compat**:flag default OFF → node 永不 invoked,workflow.py 还未引用 r1b_loop。Pure additive module。**Phase 2 5/5 + Phase 3 6/8 + R8-v2 3/6 + flat-F4 Step 1 + Q10 12/12 COMPLETE + R1b.1 (2/4) shipped = master plan v1.43 路线 ~98.5% 推进** |
| 2026-05-18 | v1.42 | **Phase 3 R1b.1a ship — foundational scaffolding (5 flags + R1bRetryLog + Alembic R1b-A)**(R1b 5-sub-phase / 24.5d plan v1.3 implementation kickoff,1/4 R1b.1 PRs)。**Scope**:mirror Q10 PR1b pattern — config + flags + model + Alembic foundational layer。**`backend/config.py`** +5 flags(`ENABLE_R1B_RETRY_LOOP` R1b.1 / `ENABLE_R1B_HYPOTHESIS_MUTATE` R1b.2 / `ENABLE_R1B_FAILURE_TREE` R1b.3 / `ENABLE_R1B_TYPED_PIPELINE` R1b.4 / `ENABLE_R1B_DAG_RETRY_REWARD` R1b.5)+ 6 scalars(`R1B_MAX_RETRIES_PER_ALPHA=3` / `R1B_MAX_MUTATIONS_PER_DATASET_CYCLE=2` / `R1B_FAILURE_TREE_MAX_DEPTH=4` / `R1B_RETRY_MODEL=claude-haiku-4-5-20251001` / `R1B_MUTATE_MODEL` / `R1B_TOKEN_COST_CEILING_USD_PER_ALPHA=0.05` shared budget guard);**`feature_flag_service.py`** SUPPORTED_FLAGS 双文件注册 5 entries Phase3-R1b group per `[[feedback_enable_flag_double_file]]`(34 total flags);**`backend/models/r1b_retry.py`** NEW R1bRetryLog table(23 cols / 4 indexes ix_r1b_task_id/created_at/attempt_type/outcome)— dedicated per `[[feedback_r1a_dedicated_log_table]]` 因 R1b fires per FAIL alpha typed-attribution,outcome 列(outcome / outcome_alpha_id_brain / outcome_sharpe / outcome_fitness)由 post-BRAIN follow-up reconciliation 填,pending 直到 next iteration;**Alembic `d6f8a3b1e9c4`** revises Q10 head `c5d9e1f3a7b8` →(1)create r1b_retry_log table + 4 indexes(2)ADD `parent_hypothesis_id` INTEGER FK self-ref ON DELETE SET NULL + `r1b_mutation_depth` INTEGER DEFAULT 0 on hypothesis table for R1b.2 mutation chain audit。**Smoke verified**:Alembic head 移 `d6f8a3b1e9c4` / R1bRetryLog import OK 23 cols / settings defaults correct(R1B_MAX_RETRIES_PER_ALPHA=3 / R1B_TOKEN_COST_CEILING_USD_PER_ALPHA=0.05)。**Updated** `test_q10_alembic.py::test_alembic_q10_revision_in_chain`(was head check)— R1b-A pushes head forward,Q10 现在 chain 而非 head;walk_revisions still verifies file existence。**Backward-compat**:all 5 R1b flags default OFF + 模型/migration 纯 additive → zero production behavior change。**R1b.1 progress 1/4 PRs**(R1b.1a `9f506b9` foundational);剩 R1b.1b r1b_loop.py + retry/mutate prompts + R1b.1c workflow edges/routers + R1b.1d 12 unit tests + GO gate observation。**R1b 5-sub-phase total**:R1b.1 retry loop(5d)+ R1b.2 hypothesis mutate(5d)+ R1b.3 failure trees(4d)+ R1b.4 typed pipeline(5.5d)+ R1b.5 DAG retry reward(3d)+ cross-cutting(2d)= **24.5 人日 ≈ 5 周**(within plan §4.7 4-6 周 ceiling)。**Phase 2 5/5 + Phase 3 6/8 + R8-v2 3/6 + flat-F4 Step 1 + Q10 12/12 COMPLETE + R1b.1a (1/4) shipped = master plan v1.42 路线 ~98% 推进** |
| 2026-05-18 | v1.41 | **🎉 Phase 3 Q10 SHIP COMPLETE — PR2e alembic upgrade/downgrade smoke + 6 tests (12/12 files,Q10 全部 ship,Phase 3 6/8 task complete)**(plan v1.3 §7.6 final migration safety net)。**Scope**:`backend/tests/migration/test_q10_alembic.py` NEW(~140 LoC)+ 6 tests covering 两 layers:(1) **Module-level smoke 4 tests run WITHOUT PG**:migration file imports cleanly / revision IDs match(c5d9e1f3a7b8 / down_revision b3c8d9e2f4a1 R8 GIN)/ upgrade + downgrade sources reference `qlib_prescreen_log` table + all 4 indexes(ix_q10_task_id/created_at/verdict/expr_hash)/ Alembic head resolves to Q10 revision。Catches typos / wrong revision chaining in CI without Postgres。(2) **2 requires_postgres DDL tests**(skipped when PG_TEST_DSN unset per conftest.py [V1.2-C3]):upgrade verifies table + 4 indexes exist via inspect / downgrade verifies table dropped cleanly。**4 PASS + 2 SKIPPED**(PG-gated graceful)。**Q10 ALGORITHM CHAIN COMPLETE 12/12 files**:**(1) translator + reverse**(PR1a `ece51e1`)+ **(2) foundational** config + flags + log model + Alembic(PR1b `7d35a6d`)+ **(3) prescreen skeleton + QlibEngine probe**(PR1c `eabcd82`)+ **(4) pandas evaluator** 17 ops dispatch(PR1d `63c7186`)+ **(5) snapshot loader + wire pandas engine**(PR1e `0d839ea`)+ **(6) metric closure** _compute_ic_and_sharpe + forward returns(PR1f `2cce883`)+ **(7) node_simulate wire** 3-mode dispatch(PR2a `41a6231`)+ **(8) refresh_qlib_snapshot.py CLI**(PR2b `bde80fe`)+ **(9) calibrate_qlib_prescreen.py Pareto recommender**(PR2c `bd21803`)+ **(10) q10_layer_telemetry_report.py daily Slack cron**(PR2d `ee1786a`)+ **(11+12) test_q10_alembic.py upgrade/downgrade smoke**(PR2e `4fc7667`)。**140 PASS + 2 SKIPPED Q10 final cumulative**(113 unit + 4 integration + 4+2 alembic = comprehensive)。**Production deploy ready**:1 cmd `python scripts/refresh_qlib_snapshot.py --regions USA --source pyqlib`(production)or `--source synthetic`(dev)+ 1 flag flip `/ops/feature-flags PATCH ENABLE_QLIB_PRESCREEN=true` + 3-stage rollout `QLIB_PRESCREEN_MODE` shadow 5d → soft 3d → hard once GO gate(brain_disagreement ≤ 15% + cost reduction ≥ 25%)。**Cron setup**:daily 09:00 SH telemetry / weekly Sun 02:00 SH calibration / quarterly manual refresh per plan §9。**Phase 3 task progress 6/8**:flat-F1/F2/F3 + R9 + R8 + **Q10** ✓;剩 R1b(5-sub-phase plan v1.3 ready,24.5d multi-session)+ phase15-D。**Phase 2 5/5 + Phase 3 6/8 + R8-v2 3/6 + flat-F4 Step 1 + Q10 COMPLETE 12/12 + R1b plan v1.3 ready = master plan v1.41 路线 ~98% 推进**(剩 ~5d R1b 多 session + phase15-D + flat-F4 Step 2-5 operator decision) |
| 2026-05-18 | v1.40 | **Phase 3 Q10 PR2d ship — q10_layer_telemetry_report.py daily Slack cron + 13 tests (operator toolkit triple complete)**(10/12 files;Q10 plan v1.3 §9 + §15.2 daily cron deliverable)。**Scope**:`scripts/q10_layer_telemetry_report.py` NEW(~250 LoC)+ 13 unit tests — aggregates last N hours `qlib_prescreen_log` into ops summary:verdict breakdown(pass/reject/skip)+ mode breakdown(shadow/soft/hard)+ engine tier distribution + median + p99 elapsed_ms(latency contract ≤200ms target)+ cost_saved%(只算 hard-mode rejects / total — shadow + soft rejects 仍 hit BRAIN)+ translation_success_pct(1 - untranslatable_count / total)+ fn_rate(只 brain_followup_disagreement ≥10 才报)。**Three alert levels per §9**:**ALERT**(fn_rate > 0.15 — Q10 wrongly rejecting alphas BRAIN would PASS;ACTION:demote QLIB_PRESCREEN_MODE)/ **INFO**(cost_saved < 10% OR total_rows=0;ACTION:lower floor / verify flag + task running)/ **OK**(healthy)。Optional `--slack-webhook` POSTs report as code block via stdlib urllib(no extra deps)。`--exit-nonzero-on-alert` returns rc=2 for paging。`--rows-json` test path bypasses DB。**13/13 PR2d tests PASS**:empty / verdict + mode breakdown / cost_saved 只算 hard rejects / translation success / fn_rate min-followups 10 / latency p99 / 3 alert levels(ALERT high fn / INFO low cost / OK healthy)/ format_report empty + alert action advice / CLI rows-json prints + --exit-nonzero-on-alert returns rc=2。**Live smoke**:28 synthetic rows(15 pass shadow + 8 reject hard + 5 untranslatable)→ OK level + 28.57% cost saved + 82.14% translate success + insufficient followup msg。**136/136 Q10 cumulative PASS**(123 + 13 new telemetry)。**Q10 operator toolkit TRIPLE COMPLETE**(refresh + calibrate + **telemetry**):**production cron setup**:(1) daily 09:00 SH `q10_layer_telemetry_report.py --slack-webhook $Q10_SLACK_WEBHOOK`(2) weekly Sun 02:00 SH `calibrate_qlib_prescreen.py --lookback-days 14`(3) quarterly manual `refresh_qlib_snapshot.py --regions USA,CHN --source pyqlib` per §9 data-licensing operator-approval requirement。**Q10 PR1+PR2 progress 10/12 files**(`ece51e1`+`7d35a6d`+`eabcd82`+`63c7186`+`0d839ea`+`2cce883`+`41a6231`+`bde80fe`+`bd21803`+**`ee1786a`** = translator + foundational + skeleton + pandas engine + pipeline wiring + metric closure + node_simulate wire + deploy script + calibration recommender + **telemetry report**)。剩 2 files:test_q10_alembic.py(upgrade/downgrade smoke 2 tests)。**Phase 2 5/5 + Phase 3 5/8 + R8-v2 3/6 + flat-F4 Step 1 + Q10 (10/12 operator toolkit complete) + R1b plan v1.3 ready = master plan v1.40 路线 ~97% 推进** |
| 2026-05-18 | v1.39 | **Phase 3 Q10 PR2c ship — calibrate_qlib_prescreen.py Pareto recommender + 10 tests**(9/12 files;Q10 plan v1.3 §4.5 + §13.2 + §13.3 operator-in-loop)。**Scope**:`scripts/calibrate_qlib_prescreen.py` NEW(~220 LoC)+ 10 unit tests — weekly cron piece replays last N days `qlib_prescreen_log` JOIN `alphas` on brain_expression for (local_sharpe, brain_passed) pairs → sweep floor ∈ [0.1, 0.5] step 0.05 → 2×2 confusion matrix per floor(TP kept good / FN Q10 wrongly rejected / FP Q10 missed loser / TN saved cost)→ cost_saved% = TN/total + fn_rate = FN/(FN+TP)→ recommend highest floor whose fn_rate ≤ --max-fn-rate (default 0.15)。**Operator-in-loop** per §13.3:script NEVER auto-flips,recommendation goes to stdout / Slack,operator manually PATCH /ops/feature-flags if drift > 0.05 vs current。**Insufficient data** (<--min-samples) → exit 0 graceful per [V1.2-A2-12]。CLI:`--lookback-days 14` `--min-samples 50` `--max-fn-rate 0.15` `--floor-min 0.1` `--floor-max 0.5` `--floor-step 0.05` `--pairs-json`(test path bypasses DB)。**10/10 PR2c tests PASS**(2x2 classification correctness / empty pairs / sweep step count 9 / recommend max-cost-saved within gate / no candidate meets gate → None / format_report RECOMMEND marker + no-recommendation message / load_pairs_from_json invalid-row drop / CLI insufficient → rc 0 / CLI adequate → prints Pareto table)。**Live CLI smoke**:100 synthetic well-separated pairs at sharpe=0.4 → script recommends floor=0.350 with 40% cost saved + 0 FN single CLI call。**123/123 Q10 cumulative PASS**(113 + 10 new calibrate)。**Q10 PR1+PR2 progress 9/12 files**(`ece51e1`+`7d35a6d`+`eabcd82`+`63c7186`+`0d839ea`+`2cce883`+`41a6231`+`bde80fe`+**`bd21803`** = translator + foundational + skeleton + pandas engine + pipeline wiring + metric closure + node_simulate wire + deploy script + **calibration recommender**)。剩 3 files:`scripts/q10_layer_telemetry_report.py`(daily Slack)+ 2 alembic tests(test_q10_alembic.py)。**Phase 2 5/5 + Phase 3 5/8 + R8-v2 3/6 + flat-F4 Step 1 + Q10 (9/12 deployable + calibratable) + R1b plan v1.3 ready = master plan v1.39 路线 ~96% 推进** |
| 2026-05-18 | v1.38 | **Phase 3 Q10 PR2b ship — refresh_qlib_snapshot.py CLI + 4 tests (Q10 deployable)**(8/12 PR1+PR2 files;Q10 plan v1.3 §3.2 + §9 production-deploy unlock)。**Scope**:`scripts/refresh_qlib_snapshot.py` NEW(~190 LoC)— CLI 两 modes:`pyqlib`(real qlib.D.features OHLCV,top-N by recent 20-day volume,last `--years` × 252 business days)+ `synthetic`(deterministic random walk np.random seed=42 for dev/CI without pyqlib install)。`auto` mode tries pyqlib first → falls back to synthetic;explicit `--source pyqlib` returns rc=1 when pyqlib unavailable(deterministic for ops scripts)。CLI args:`--regions USA,CHN` `--source auto|pyqlib|synthetic` `--years 5` `--top 500` `--snapshot-dir` `--qlib-data-dir`(后两个 override settings)。Writes `{QLIB_SNAPSHOT_DIR}/{REGION}.parquet` 匹配 `QlibEngine._load_snapshot` contract:MultiIndex (datetime, instrument) + OHLCV columns。**Production deploy cycle now end-to-end runnable**:(1) `python scripts/refresh_qlib_snapshot.py --regions USA --source pyqlib`(production)or `--source synthetic`(dev)(2) `/ops/feature-flags` PATCH `ENABLE_QLIB_PRESCREEN=true`(3) `QLIB_PRESCREEN_MODE='shadow'` for 5d(plan §10 stage 1→2 gate)(4) Promote 'soft' for 3d once shadow GO(5) Promote 'hard' once GO gate(brain_disagreement ≤15% / cost reduction ≥25%)。Quarterly per plan §9 operator-manual NOT auto-scheduled(data licensing concerns)。**4/4 PR2b tests PASS**(synthetic write → valid MultiIndex parquet + 6 OHLCV cols + 20 instruments / CLI multi-region invocation writes both USA + CHN / explicit pyqlib mode handles missing install gracefully rc ∈ {0,1} / end-to-end snapshot → QlibEngine probes pandas_snapshot → prescreen_alpha returns verdict ∈ {pass, reject})。**CLI live smoke**:`python scripts/refresh_qlib_snapshot.py --source synthetic --regions USA --top 10` 生成 2520-row × 10-instrument × 252-day parquet 100ms 143KB。**113/113 Q10 cumulative PASS**(35 forward + 20 reverse translator + 14 skeleton + 13 pandas engine + 13 pipeline + 10 metrics + 4 wire + 4 refresh script)。**Q10 PR1+PR2 progress 8/12 files**(`ece51e1`+`7d35a6d`+`eabcd82`+`63c7186`+`0d839ea`+`2cce883`+`41a6231`+**`bde80fe`** = translator + foundational + skeleton + pandas engine + pipeline wiring + metric closure + node_simulate wire + **deploy script**)。**Q10 现 fully deployable in production**(只需 1 命令生成 snapshot + 1 flag flip)。剩 4 files:`scripts/calibrate_qlib_prescreen.py`(plan §4.5 quantile floor calibration cron)+ `scripts/q10_layer_telemetry_report.py`(daily Slack report cron)+ 2 alembic tests(test_q10_alembic.py)。**Phase 2 5/5 + Phase 3 5/8 + R8-v2 3/6 + flat-F4 Step 1 + Q10 PR1a-f+PR2a+PR2b (8/12 deployable) + R1b plan v1.3 ready = master plan v1.38 路线 ~95% 推进** |
| 2026-05-18 | v1.37 | **Phase 3 Q10 PR2a ship — wire prescreen_alpha into node_simulate + 4 integration tests (Q10 active-latent in production)**(7/12 PR1+PR2 files;Q10 plan v1.3 §5 + §12 ordering lock + §7.4 mode tests subset)。**Scope**:`backend/agents/graph/nodes/evaluation.py` 加 `hashlib` import + Q10 block inserted BEFORE `ENABLE_PRE_SIMULATE_FILTER` block(+~90 LoC)— per plan §12 ordering rationale Q10 reject 是 FREE local eval 且 stacks before R9 cache + BRAIN sim,asymmetric-cost analysis:rejected alpha saves cache lookup + BRAIN call + cache write,cache-hit doesn't save Q10 either。**Three-mode behavior**(`QLIB_PRESCREEN_MODE` read ONCE at entry — mid-batch flag flip race per [V1.2-A2-3]):shadow log only/ soft log + `alpha.metrics["_qlib_prescreen_warned"]=True` + local_sharpe/ic stamp on reject keeping BRAIN/ hard pull index from `indices_to_simulate` + mark `simulation_success=False` + `simulation_error="Q10 pre-screen reject: <reason>"`(mirror existing pre_simulate_filter shape)。Per-alpha try/except — failure never blocks round。Top-level block also wrapped soft-fall。Batch INSERT to `qlib_prescreen_log` via dedicated `AsyncSessionLocal` per plan §6.4 — independent of round transaction,失败 log warn 不阻塞。**4/4 PR2a integration tests PASS**(shadow logs all + zero drops + no warned stamp / soft stamps warned + sharpe + ic on reject but keeps indices / hard drops rejected index + marks simulation_success=False + simulation_error contains "Q10" / skip verdict any reason NEVER drops regardless of mode — protects untranslatable + engine_disabled paths from accidentally killing BRAIN call)。**109/109 Q10 cumulative PASS**(105 unit + 4 integration)。**Q10 ACTIVE-LATENT in production**:`ENABLE_QLIB_PRESCREEN=False` default → block 完全 skipped(zero behavior change)。Flag flip + snapshot deploy → Q10 fires immediately in chosen mode。**Production deployment sequence**:(1) `scripts/refresh_qlib_snapshot.py` 跑一次 populate `QLIB_SNAPSHOT_DIR/{REGION}.parquet`(待 PR ship)(2) `/ops/feature-flags` PATCH `ENABLE_QLIB_PRESCREEN=true`(3) Start shadow 5 days(plan §10 stage 1→2)→ promote soft 3 days → promote hard once GO gate confirmed(brain_disagreement ≤15% / cost reduction ≥25%)。**Q10 PR1+PR2a progress 7/12 files**(`ece51e1`+`7d35a6d`+`eabcd82`+`63c7186`+`0d839ea`+`2cce883`+**`41a6231`** = translator + foundational + skeleton + pandas engine + pipeline wiring + metric closure + **node_simulate wire**)。剩 5 files:`scripts/refresh_qlib_snapshot.py` + `scripts/calibrate_qlib_prescreen.py` + `scripts/q10_layer_telemetry_report.py` + 2 alembic tests。**Phase 2 5/5 + Phase 3 5/8 + R8-v2 3/6 + flat-F4 Step 1 + Q10 PR1a-f+PR2a (7/12 active-latent in production) + R1b plan v1.3 ready = master plan v1.37 路线 ~94% 推进** |
| 2026-05-18 | v1.36 | **Phase 3 Q10 PR1f ship — close metric loop:_compute_ic_and_sharpe + forward returns + 10 tests (Q10 algorithm chain 100% closure)**(6/12 PR1 files;Q10 plan v1.3 §4.2 implementation 完整)。**Scope**:`backend/qlib_prescreen.py` 改(+~80 LoC)— `QlibEngine.get_forward_returns(region)` per-instrument 1-day forward return = close.shift(-1)/close - 1,cached per region(key `__fwd_ret_{REGION}`),soft-fail None on missing snapshot / `_compute_ic_and_sharpe(signal, fwd)` pandas impl per plan §4.2:(1) align signal + fwd on shared MultiIndex inner join,(2) cross-section pct rank centered on 0 per datetime,(3) unit-gross long-short weights `w = rank/rank.abs().sum()` per day,(4) **lag weights by 1 day per instrument** 避免 look-ahead bias(critical),(5) daily PnL = sum(w[t-1] × fwd_return[t]) per datetime,(6) Annualized Sharpe = mean(PnL)/std(PnL) × sqrt(252),(7) per-day Spearman IC averaged for IC。NaN/inf sanitized → None。Defensive:returns (None, None) on <5 (date,instrument) pairs / zero PnL std / numpy or pandas missing / numerical exception。**prescreen_alpha Step 4 现真 wire** to `engine.get_forward_returns(region)` + `_compute_ic_and_sharpe`,4-stage transition 完整闭环:engine_disabled → empty_series → metrics_nan(PR1f kill this)→ **pass / reject** real verdict。**10/10 PR1f tests PASS** in `test_qlib_prescreen_metrics.py`:3 forward returns(shape last-row NaN + cache same-object + missing-region None)+ 2 _compute_ic_and_sharpe direct((None,None) + 3-pair short input None)+ 5 verdict closure(random signal pass-or-reject with float metrics / extremely low floor → pass / extremely high floor → reject with reject_reason / per-call sharpe_floor+ic_floor kwarg overrides settings / metrics populated as float)。**Modified 1 PR1e test** test_prescreen_alpha_pandas_tier_reaches_evaluate:from 'expects skip:metrics_nan' to 'expects pass-or-reject with finite metrics' — 同 fixture post-PR1f wiring 自然产 real verdict(synthetic 60-day x 10-instrument strong_signal fixture passes;40-day x 5-instrument random_signal fixture typically rejects)。**105/105 Q10 PR1 cumulative PASS**(35 forward + 20 reverse translator + 14 prescreen skeleton + 13 pandas engine + 13 pipeline + 10 metrics)。**Backward-compat**:default `ENABLE_QLIB_PRESCREEN=False` + no snapshot → engine='disabled' → zero production behavior change。**Q10 PR1 progress 6/12 files**(`ece51e1`+`7d35a6d`+`eabcd82`+`63c7186`+`0d839ea`+**`2cce883`** = translator + foundational + skeleton + pandas engine + pipeline wiring + **metric loop closure**)。**Q10 algorithm chain 100% complete** — `prescreen_alpha("ts_mean(close, 5)", region="USA")` 与 snapshot 部署后真返 verdict ∈ {pass, reject, skip} with calibrated thresholds。剩 PR1 6 files:`scripts/refresh_qlib_snapshot.py`(production OHLCV snapshot 生成)+ `scripts/calibrate_qlib_prescreen.py`(plan §4.5 quantile floor recommendation)+ `scripts/q10_layer_telemetry_report.py`(daily cron)+ 2 alembic tests + 6 e2e tests(real BRAIN→qlib→eval→verdict path with mock BRAIN sim for ground truth)。**Phase 2 5/5 + Phase 3 5/8 + R8-v2 3/6 + flat-F4 Step 1 + Q10 PR1a-f (6/12,核心算法闭环) + R1b plan v1.3 ready = master plan v1.36 路线 ~93% 推进** |
| 2026-05-18 | v1.35 | **Phase 3 Q10 PR1e ship — snapshot loader + wire pandas engine into QlibEngine + 13 pipeline tests**(5/12 PR1 files;Q10 plan v1.3 §3.2 + §3.3 tier-3 activation)。**Scope**:`backend/qlib_prescreen.py` 改 `QlibEngine`(+~70 LoC)— `_probe_engine` 升级:tier-1/2 pyqlib 暂保 skeleton(PR2),tier-3 pandas_snapshot 当 `QLIB_SNAPSHOT_DIR/{region}.parquet` 存在或 `QLIB_ENGINE_PREFER_PANDAS=True` (dev/CI escape) 时返 'pandas_snapshot';tier-4 disabled fallback / `_load_snapshot(region)` per-region lazy load + in-memory cache keyed on uppercased region(`self._snapshots` dict),read parquet via pandas MultiIndex (datetime, instrument),missing file → None / `evaluate(qlib_expr, region, universe)` 当 kind='pandas_snapshot' 时 delegate 到 `evaluate_pandas`,import + eval + empty-df 全 soft-fail → None,kind!='pandas_snapshot' 直接 None。**prescreen_alpha 状态机现 3-stage transition**:engine_disabled(no snapshot dir)→ pandas_snapshot+metrics_nan(snapshot present + _compute_ic_and_sharpe stub returns None,None — PR1f wires real impl)→ pandas_snapshot+empty_series(known region without snapshot file)。**13/13 PR1e pipeline tests PASS**:3 probe(file present → pandas_snapshot upgrade,QLIB_ENGINE_PREFER_PANDAS forces overrides missing file,missing dir → disabled)+ 3 loader(returns df / cached per-region same object / missing region None)+ 3 engine.evaluate(runs pandas with snapshot,returns None missing region,returns None disabled)+ 4 prescreen end-to-end(engine_disabled still skips / pandas tier reaches eval with metrics_nan / untranslatable short-circuits BEFORE engine probe / known engine + unknown region → empty_series)。Synthetic tmp_path USA.parquet 30 days × 5 instruments fixture(np.random seed=7 deterministic)+ monkeypatch settings。**95/95 Q10 PR1 cumulative PASS**(35 forward + 20 reverse + 14 skeleton + 13 pandas engine + 13 pipeline)。**Backward-compat**:default `ENABLE_QLIB_PRESCREEN=False` + no snapshot → engine='disabled' → zero behavior change in production。**Q10 PR1 progress 5/12 files**(`ece51e1`+`7d35a6d`+`eabcd82`+`63c7186`+**`0d839ea`** = translator + foundational + skeleton + pandas engine + pipeline wiring);剩 PR1 7 files:`_compute_ic_and_sharpe` pandas impl(PR1f 最关键,真把 skip:metrics_nan 转 pass/reject)+ scripts/refresh_qlib_snapshot.py / scripts/calibrate_qlib_prescreen.py / scripts/q10_layer_telemetry_report.py + 2 alembic tests + 6 e2e。**Phase 2 5/5 + Phase 3 5/8 + R8-v2 3/6 + flat-F4 Step 1 + Q10 PR1a-e (5/12) + R1b plan ready = master plan v1.35 路线 ~92% 推进** |
| 2026-05-18 | v1.34 | **Phase 3 Q10 PR1d ship — pandas-snapshot evaluator + 13 unit tests**(4/12 PR1 files;Q10 plan v1.3 implementation continues per `~/.claude/plans/phase3-q10-pyqlib-prescreen-2026-05-18.md` v1.3 §3.4 + §7.3)。**Scope**:`backend/qlib_prescreen_pandas_engine.py` NEW(~280 LoC pure-pandas)— tier-3 fallback engine activated when pyqlib unavailable(common on Windows dev per `[[reference_phase0_plan_and_pyqlib_caveat]]`)。**17 ops via dispatch table**(scope-locked to plan §2.2 reverse subset per [V1.3-A2-1]):rolling per-instrument Mean/Std/Max/Min/Ref/Delta/ZScore + binary-window Corr/Cov + arity-switching Rank(1-arg cross-section pct rank centered 0,2-arg time-series rank via groupby+rolling argsort)+ elementwise Add/Sub/Mul/Div(自动 _as_series 标量 promote + Div /0 → NaN)+ unary Abs/Sign/Log/Sqrt(negative/zero → NaN safe)。**Unknown op cascade to None**(WMA/Slope/SignedPower/Quantile 等 plan 外 ops 返 None,caller skip:eval_error)。**Defensive pandas import**:`_try_import_pandas()` defer 到 first call,module 在无 pandas CI 也能 load。**MultiIndex (datetime, instrument)** OHLCV df contract。**evaluate_pandas NEVER raises**:top-level try/except 包裹 recursive eval;impl-level errors(op raised)也返 None。**13/13 PR1d tests PASS**(plan §7.3 subset:Mean rolling first-row partial-window correct / Std rolling late-row >0 / Ref shift / Delta first-window NaN / Rank cross-section 5 distinct ranks in [-0.5, 0.5] / nested Mul(Div(...)) ~100 / unknown op None / unknown field None / empty/None expression / unbalanced paren / empty df / arity mismatch / monkeypatched op-impl exception 安全 None)。Synthetic 30-row x 5-instrument OHLCV fixture(np.random seed=42 deterministic)。**82/82 Q10 PR1 cumulative PASS**(35 forward translator + 20 reverse + 14 prescreen skeleton + 13 pandas engine)。**Backward-compat**:`evaluate_pandas` 还未 wired 到 `QlibEngine.evaluate` — PR1e 加 snapshot loader 把 engine.kind 升到 `pandas_snapshot`;PR2 加 pyqlib live tier。Pure additive module。**Q10 PR1 progress 4/12 files**(`ece51e1`+`7d35a6d`+`eabcd82`+**`63c7186`** = translator + foundational + skeleton + pandas engine);剩 PR1 8 files:snapshot loader / pyqlib live tier probe wire / 6 e2e tests / 2 alembic tests / refresh_qlib_snapshot.py / calibrate script / telemetry report script。**Phase 2 5/5 + Phase 3 5/8 + R8-v2 3/6 + flat-F4 Step 1 + Q10 PR1a+b+c+d (4/12) + R1b plan ready = master plan v1.34 路线 ~91% 推进** |
| 2026-05-18 | v1.33 | **Phase 3 Q10 PR1c ship — prescreen_alpha skeleton + QlibEngine probe + 14 tests**(3/12 PR1 files;Q10 plan v1.3 implementation continues per `~/.claude/plans/phase3-q10-pyqlib-prescreen-2026-05-18.md` §3-§4)。**Scope**:`backend/qlib_prescreen.py` NEW(~245 LoC)— `PrescreenResult` dataclass(brain_expression / region / universe / verdict ∈ pass|reject|skip / reject_reason / skip_reason / translation_error / qlib_expression / local_sharpe / local_ic / engine_kind / elapsed_ms / mode_at_call / extra dict)+ `QlibEngine` class skeleton(_probe_engine 默认返 'disabled' until PR1d pandas-snapshot 与 PR2 pyqlib live+snapshot tier 加 eval;singleton via `_get_engine()` lazy build,`_reset_engine_for_test()` 提供测试隔离)+ `_compute_ic_and_sharpe(signal, fwd_returns)` stub(PR1d 加 pandas implementation:cross-section rank → unit-gross weights → daily PnL → Spearman IC + annualized Sharpe × sqrt(252))+ **`async prescreen_alpha(brain_expr, region, universe, *, sharpe_floor, ic_floor, mode)` public entry**(Step 1 translate via `brain_to_qlib` → untranslatable 返 skip:untranslatable / Step 2 engine probe → disabled 返 skip:engine_disabled / Step 3-5 metric compute + verdict 在 PR1c 走 stub 返 skip:empty_series / never-raises 包顶层 try/except 返 skip:eval_error:`<ExcType>`)。**Skeleton ship**:engine always 'disabled' → prescreen_alpha 始终返 skip(translation succeeds → skip:engine_disabled / translation fails → skip:untranslatable)。**Soft-fail contract**:函数承诺 never raises;`prescreen_alpha(None)` + 不平衡 paren + unknown field 等 broken input 都 graceful 返 skip。**14/14 PR1c unit tests PASS**(plan §7.2 coverage subset:translatable+disabled engine → skip:engine_disabled / untranslatable cascade → skip:untranslatable / empty/unknown-field/unbalanced-paren defensive / elapsed_ms populated / local metrics None on skip / mode_at_call from settings + override / never-raises on None / engine probe returns disabled / engine.evaluate returns None / singleton reusable + _reset hook / dataclass defaults)+ **55/55 translator regression PASS** = **69/69 cumulative Q10 PASS**。**Backward-compat**:`prescreen_alpha` 还未 wired 到 `node_simulate` — PR2 才 wire。Pure additive module。**Q10 PR1 progress 3/12 files**(`ece51e1` PR1a translator + `7d35a6d` PR1b foundational + **`eabcd82` PR1c skeleton**);剩 PR1 9 files:`qlib_prescreen_pandas_engine.py` 250 LoC eval(PR1d最大)+ 8 pandas parity tests + `scripts/refresh_qlib_snapshot.py` + 6 e2e + 2 alembic tests。**Phase 2 5/5 + Phase 3 5/8 + R8-v2 3/6 + flat-F4 Step 1 + Q10 PR1a+b+c (3/12) + R1b plan ready = master plan v1.33 路线 ~90% 推进** |
| 2026-05-18 | v1.32 | **Phase 3 Q10 PR1b ship — config + flags + log model + Alembic foundational**(2/12 PR1 files;Q10 plan v1.3 5d implementation 继续推进 per `~/.claude/plans/phase3-q10-pyqlib-prescreen-2026-05-18.md` §6)。**Scope**:`backend/config.py` +8 settings(`ENABLE_QLIB_PRESCREEN: bool = False` + `QLIB_PRESCREEN_MODE: str = "shadow"`(3-stage rollout shadow|soft|hard)+ `QLIB_PRESCREEN_SHARPE_FLOOR=0.3` + `QLIB_PRESCREEN_IC_FLOOR=0.005` + `QLIB_PRESCREEN_TIMEOUT_MS=1500` + `QLIB_DATA_DIR` + `QLIB_SNAPSHOT_DIR` + `QLIB_ENGINE_PREFER_PANDAS=False`)+ `backend/services/feature_flag_service.py` SUPPORTED_FLAGS 双文件注册 `ENABLE_QLIB_PRESCREEN` + `QLIB_PRESCREEN_MODE`(Phase3-Q10 group)+ `backend/models/qlib_prescreen_log.py` NEW(21 cols / 4 indexes / dedicated table per `[[feedback_r1a_dedicated_log_table]]` — Q10 fires per simulate attempt incl. skipped/untranslatable,50× INSERT throughput vs piggyback on alpha.metrics;brain_followup_status/sharpe/disagreement 三列 NULL in hard mode where BRAIN was never called)+ `backend/alembic/versions/c5d9e1f3a7b8_phase3_q10_qlib_prescreen_log.py` NEW(revises `b3c8d9e2f4a1` R8 GIN → new head `c5d9e1f3a7b8`,create_table + 4 indexes ix_q10_task_id/created_at/verdict/expr_hash for cross-tab calibration queries,downgrade clean DROP TABLE — zero-risk additive)。**Smoke-verified**:`from backend.models.qlib_prescreen_log import QlibPrescreenLog` OK(21 cols),`settings.ENABLE_QLIB_PRESCREEN=False` + `settings.QLIB_PRESCREEN_MODE='shadow'` defaults correct,Alembic head moved to `c5d9e1f3a7b8`,**55/55 translator cumulative PASS**(20 reverse + 35 forward,no regression)。**Backward-compat**:flag ON 任何切换 zero behavior change(无 caller wired — PR2 才 wire `node_simulate` Q10 block + `_run_one_round_typed_pipeline` integration with `multi_fidelity_eval.LOCAL` stage)。**Q10 PR1 progress 2/12 files**(PR1a `ece51e1` translator + PR1b `7d35a6d` foundational);剩 PR1 10 files:`qlib_prescreen.py` QlibEngine 3-tier degrade + `qlib_prescreen_pandas_engine.py` 250 LoC eval + `scripts/refresh_qlib_snapshot.py` + 12 prescreen + 8 parity + 2 alembic + 6 e2e tests。**ENABLE_QLIB_PRESCREEN production opt-in**:DB flag override 立可 flip 但当前无 caller wired(no behavior change);production canary 等 PR2 ship 后 1-task shadow mode 24h 观察。**R1b plan v1.3 同 session ready**(`~/.claude/plans/phase3-r1b-costeer-loop-2026-05-18.md`,5 sub-phases / 24.5 人日 / 28 MUST + 24 SHOULD inline-fixed)。**Phase 2 5/5 + Phase 3 5/8 + R8-v2 3/6 + flat-F4 Step 1 + Q10 PR1a+PR1b + R1b plan ready = master plan v1.32 路线 ~89% 推进** |
| 2026-05-18 | v1.31 | **Phase 3 Q10 PR1a ship — brain_to_qlib reverse translator**(first piece of 5d Q10 multi-fidelity Layer 0 plan implementation per `~/.claude/plans/phase3-q10-pyqlib-prescreen-2026-05-18.md` v1.3 §2.2-§2.4 + §7.1)。**Scope**:`backend/qlib_translator.py` 加 reverse direction(BRAIN→qlib)— `BRAIN_TO_QLIB_OPERATORS` dict (~30 entries 含 12 explicit None untranslatable per plan §2.2 REJECT rows:group_neutralize/group_rank/group_zscore/group_mean/group_scale/trade_when/vector_neut/regression_neut/indneutralize/winsorize/rank_by_side/scale)+ `BRAIN_TO_QLIB_FIELD` dict (~13 entries:6 OHLCV $-prefix + 2 synthetics adv20→Mean($volume,20) & returns→Ref($close,-1)/$close-1 + 5 explicit None whitelist cascade fnd6/fnd28/cap/pv6/pv13)+ helpers `_brain_split_args` / `_brain_field_to_qlib` whitelist lookup / `_brain_replace_leaf_fields` cascade-to-untranslatable / `_brain_convert_ts_delay` TRAP #1 sign flip (`ts_delay(x,N) → Ref(x,-N)`, handle pre-negative `ts_delay(x,-3) → Ref(x,3)`) / `_brain_convert_call` recursive dispatch / `_brain_to_qlib_inner` recursive translator with balanced-paren scan + `@lru_cache(maxsize=1024)` 装饰 `brain_to_qlib(brain_expr, region)` public entry per plan §2.1 [V1.2-A1-2]。**Defensive**:`brain_to_qlib` 保证 never raises — 任何 failure(unknown op / unbalanced parens / unknown field)返 None,caller(Q10 prescreen)soft-fall 到 "no opinion, proceed to BRAIN"。**20/20 PR1a tests PASS**(20 unit tests in `test_qlib_translator_reverse.py` 覆盖 plan §7.1 全部 case:ts_delay sign-flip + 5 field synthetics + 4 untranslatable cascade + nested-call + arithmetic + lru_cache hit count + region-unused-v1 + empty/unbalanced defensive + table contract sanity)+ 35/35 forward translator tests 仍 PASS(无 regression — append-only additions)= **55/55 qlib_translator cumulative PASS**。**No flag / no Alembic / no integration this PR** — Pure additive module-level helpers,任何 caller 还没 wired,zero behavior change。**Q10 PR1 progress 1/12 files**;剩 PR1 11 files queued:`qlib_prescreen.py` QlibEngine 3-tier degrade(+pandas fallback)+ `qlib_prescreen_pandas_engine.py` + 8 settings + 2 SUPPORTED_FLAGS + `qlib_prescreen_log` model + Alembic c5d9e1f3a7b8 + scripts/refresh_qlib_snapshot.py + 12 prescreen unit tests + 8 pandas-engine parity tests + 2 alembic tests + 6 e2e integration tests = 1.5 人日 PR1 total。**背景 R1b plan agent running**(4-6 周 plan for CoSTEER loop full activation,multi-session ship)。**Phase 2 5/5 + Phase 3 5/8 + R8-v2 4/6 + Q10 PR1a + flat-F4 Step 1 (master only) = master plan v1.30 路线 ~88% 推进** |
| 2026-05-18 | v1.29 | **Phase 3 R8-v2 #2 ship — per-layer Redis cache for hierarchical RAG**(R8-v2 progress 4/6,Phase 3 5/8 ship + R8-v2 4/6 follow-ups)。**Scope**:`backend/agents/hierarchical_rag.py` 加 R8-v2 #2 cache subsystem(`_rag_redis_client` + `_rag_redis_loop_id` module-level singletons,loop-aware rebuild mirror `BrainAdapter._get_slot_redis` Celery per-task loop lifecycle pattern + `_get_rag_redis()` async helper(import asyncio + redis.asyncio + settings 内 try import,fail → None;loop_changed OR loop_dead → 重建;redis.from_url(settings.REDIS_URL, decode_responses=True);catch init exception → None soft-fall)+ `_make_layer_cache_key(layer, params)` 稳定 sha256[:16](layer|json.dumps(sorted) + "ragcache:" prefix)+ `_entry_to_dict` / `_entry_from_dict` RAGEntry serializer (relevance_score float coercion)+ `_cache_get(key) -> Optional[(succ, fail)]` 软失败 None on miss/redis-down/JSON-corrupt + `_cache_set(key, succ, fail, ttl) -> None` setex 包 try/except 吞所有异常 + orchestrator 内 `_layer_call(layer_name, cache_params, fetcher)` 闭包 wrap 4 layer 调用,flag ON 时 cache check → miss 调 fetcher + write-through / hit 直接返,flag OFF 直 fetcher)。**无显式 invalidation**:KB write rate 3-50/h × TTL 5min → 5-min stale window 在 plan §10 GO gate 容忍范围;避免 SCAN/generation-bump 复杂度。**Tests** 13 new unit:4 cache key 稳定性(same params order-independent / per-layer differ / per-params differ / has prefix)+ 5 cache get/set 助手(redis None get → None / redis None set → no-op / FakeRedis round-trip preserves all fields incl. meta_data + relevance_score / corrupt JSON → None / redis.get raise → None)+ 4 orchestrator dispatch(flag OFF → 0 cache calls + layer still called / flag ON cache miss → 4 layer get + 4 set / flag ON L0 cache hit → layer0 NOT called + L1-3 still get/set (3 set vs 4) / flag ON sanity result surfaces layer output)。**配置**:`backend/config.py` `ENABLE_HIERARCHICAL_RAG_CACHE: bool = False` + 复用现有 `RAG_HIER_CACHE_TTL_SEC=300` setting;`feature_flag_service.py` SUPPORTED_FLAGS 注册(27 total)双文件改动 per `[[feedback_enable_flag_double_file]]`。**Backward-compat**:flag OFF default → orchestrator 行为完全 unchanged(无 cache call attempted);redis 不可用 → soft-fall 直 layer call。**R8-v2 progress 4/6**(#1 dispatch af03809 + #5 flat-F3 wire 194e937 + #3 R5 ranking 782ca9e + **#2 Redis cache <COMMIT>**);剩 2/6 follow-ups(#4 node_rag_query early-stage caller wire — current_expression not available at workflow position,opt-in ready / #6 production canary verify both flag ON 1 task 24h)。**Production opt-in**:ENABLE_HIERARCHICAL_RAG_CACHE 需 flip ON via DB override 才 cache(默认 OFF zero behavior change);redis 已生产可用(Celery broker),无新基础设施依赖。**13/13 R8-v2 #2 + 62/62 R8 cumulative PASS**(含 cache 影响下的 PR1+PR2+PR3 orchestrator + dispatch + ranking 全 case;pre-existing layer1_includes_decayed_failure 也现 PASS — 用户 SQL pushdown 1607c18 顺修)。**Phase 2 5/5 + Phase 3 5/8 + R8-v2 4/6 = master plan v1.29 路线 ~87% 推进** |
| 2026-05-18 | v1.28 | **Phase 3 R8-v2 #3 ship — R5 composite_score ranking in layer2_family**(R8-v2 progress 3/6,Phase 3 5/8 ship + R8-v2 3/6 follow-ups)。**Scope**:`backend/agents/hierarchical_rag.py` 加 `_expr_hash_64(expr)` helper(sha256[:64] matches `evaluation.py:2631` R1a hook convention 跨 hash 系列)+ `fetch_r5_avg_scores(db, expressions, min_samples, lookback_days)` 单 SQL `SELECT AVG GROUP BY expression_hash WHERE expression_hash = ANY(:hashes) AND r5_composite_score IS NOT NULL AND created_at > now() - :days · interval` text() stmt(asyncpg ANY 数组绑定,避 R1aAttributionLog model import cycle),返 {hash: (avg, n)} soft-fail → {} + `layer2_family(enable_r5_ranking=False, r5_min_samples=1, r5_lookback_days=30)` 新 3 参数:flag ON 时 over-fetch limit=budget·8(vs budget·4 原)以避高分 pattern 被 first-`budget` cap 之外 truncate / 拿 SUCCESS candidate pattern list → `fetch_r5_avg_scores` → 每 entry 有样本时 set `relevance_score = 0.45 + 0.4·avg` (range [0.45, 0.85]) + 标 `_r5_composite_avg` + `_r5_sample_count` 到 entry.meta_data / sort by relevance_score DESC / budget cap as before。零样本 row 保 default 0.65 neutral。**Orchestrator dispatch**:`query_hierarchical` L2 调用前读 `settings.ENABLE_R5_L2_RANKING` + `R5_L2_RANKING_MIN_SAMPLES` + `R5_L2_RANKING_LOOKBACK_DAYS` 三 settings forward 到 layer2_family,settings read fail → soft-fall flag OFF。**Tests**:8 new (3 fetch_r5_avg_scores 单测 + 5 layer2_family 集成:flag OFF preserves order + flag ON reranks by R5 (low<mid<high 排序顺序 verify) + flag ON no r5 rows preserves default + flag ON SQL error soft-falls (mock fetch raise) + flag ON partial r5 rows (seeded sorts before unseeded))。**配置**:`backend/config.py` ENABLE_R5_L2_RANKING default OFF + R5_L2_RANKING_MIN_SAMPLES=1 + R5_L2_RANKING_LOOKBACK_DAYS=30;`feature_flag_service.py` SUPPORTED_FLAGS 注册(26 total)双文件改动 per `[[feedback_enable_flag_double_file]]`。**Backward-compat**:flag OFF default 时 layer2_family signature 新 3 参数都有 default → 现有 caller 不受影响(query_hierarchical 已 forward,直 helper-tests 可省)。**R8-v2 progress 3/6**(#1 dispatch af03809 + #5 flat-F3 wire 194e937 + **#3 R5 ranking <COMMIT>**);剩 3/6 follow-ups(#2 Redis cache 5-min TTL / #4 node_rag_query early-stage caller wire — current_expression not available at workflow position / #6 production canary verify)。**Production opt-in**:ENABLE_R5_L2_RANKING 需 flip ON via DB override 才生效(默认 OFF zero behavior change);ENABLE_LLM_JUDGE 需 ≥ 1 round 累计 r5_composite_score 非 NULL 才提供信号否则 fetch 返 {} layer 行为同 flag OFF。**8/8 R8-v2 #3 tests + 48/49 R8 cumulative PASS**(1 pre-existing test_layer1_includes_decayed_failure unrelated,stash-verified)。**Phase 2 5/5 + Phase 3 5/8 + R8-v2 3/6 = master plan v1.28 路线 over 85% 推进** |
| 2026-05-18 | v1.27 | **Phase 3 R8-v2 #5 ship — flat-F3 PR2 wire R8 RAG → llm_mutate_alpha context**(同 session R8-v2 dispatch ship 后接 R8-v2 follow-up #5/6,closes flat-F3 PR2 deferred work)。commit `194e937`(43 insertions / 1 file / 0 new tests flag-gated integration / 263/263 cumulative regression PASS / ~10 min)。**Scope**:`backend/agents/graph/nodes/tier_seed.py:node_tier_wrap_one` T2 分支,在 llm_mutate_alpha 调用前 if `ENABLE_HIERARCHICAL_RAG` ON:(1) query_hierarchical(db, current_expression=seed.expression, region, max_patterns=0, max_pitfalls=8) — max_patterns=0 因 mutate prompt 不需 success refs;(2) Iterate hier.pitfalls 按 meta_data.decayed partition:decayed=true → decay_context line / false → failure_context line(format `(L2_family) <pattern> — <desc>`);(3) bound 6 failure_lines + 4 decay_lines;(4) 传给 llm_mutate_alpha 作 actual context(原为 empty default placeholder)。Soft-fall:任 R8 RAG error → log debug + empty contexts(llm_mutate falls back to "(no recent failures recorded)" default — flat-F3 PR1 original behavior)。**Without this wire**:llm_mutate_alpha 只见 placeholder strings → LLM 挑 wrapper 盲于 documented failure modes(原 flat-F3 PR1 缺口)。**Both flag ON 时**:R8 RAG → flat-F3 mutate prompt 真正 informed — Phase 3 第一对跨 task 真正 cross-talk 场景。**R8-v2 progress 2/6**(#1 dispatch af03809 + #5 flat-F3 wire 194e937);剩 4/6 follow-ups(#2 Redis cache 5-min TTL / #3 R5 composite_score JOIN for L2 ranking / #4 node_rag_query early-stage caller wire — current_expression not available at that workflow position,opt-in ready / #6 production canary verify both flag ON 1 task 24h 观察 failure_context + decay_context in mutate logs)。**Phase 3 进度 5/8 ship + R8-v2 2/6 follow-ups**;Phase 2 5/5 + Phase 3 5/8 + R8-v2 2/6 = master plan v1.27 路线 over 85% 推进|
| 2026-05-18 | v1.26 | **Phase 3 R8-v2 dispatch ship — RAGService.query() flag wiring**(同 session R8 PR3 ship 后接 R8-v2 follow-up #1/6,production opt-in 入口 ready)。commit `af03809`(259 insertions / 2 files / 5 new dispatch tests + 263 cumulative PASS / ~30 min)。**Scope**:RAGService.query() 加 2 opt-in 参数 current_expression + hypothesis_pillar — 当 ENABLE_HIERARCHICAL_RAG ON AND 任一 provided → 调 hierarchical_rag.query_hierarchical(...)+ convert RAGEntry → legacy List[Dict] shape with source_layer + relevance_score in metadata + 包 RAGService.RAGResult byte-shape parity。Soft-fall:任 hierarchical exception → log warn + fall through to legacy。Legacy callers (无 expression/pillar) 完全 unchanged opt-in 触发。**Tests**:5 dispatch (flag OFF + expression → legacy / flag ON 无 expression → legacy / flag ON + expression → hierarchical source_layer present / hierarchical raise → graceful fall / pillar-only → hierarchical 触发)。**Production flag flip ON**:ENABLE_HIERARCHICAL_RAG=true via DB override(opt-in only — 无 caller 已 wired,等 node_rag_query 之后传 current_expression 才生效)。**11 production flags** (R1a + DUAL_RAG + AST_DIM + DIR_BANDIT + FLAT_CONTINUOUS + TASK_SCHEMA_V2 + LLM_JUDGE + FAMILY_CAP + DAG_TRACE + SIMULATION_CACHE + **HIERARCHICAL_RAG**)。**R8-v2 remaining follow-ups (5 of 6)**:(2) Redis cache layer 5-min TTL + KB ingest invalidate;(3) R5 composite_score ranking via r1a_attribution_log JOIN for L2 SUCCESS re-rank;(4) node_rag_query in evaluation.py 传 current_expression at retrieval site(currently unused — opt-in dispatch ready for this hook);(5) flat-F3 PR2 wiring backend/agents/llm_mutate_alpha.py default failure_context / decay_context replaced with RAG#2 family output;(6) Production canary verify flag flip ON + 1 task 24h + per-layer hit-rate balanced (plan §10 GO gate b)。**Phase 3 进度 5/8 ship + R8-v2 1/6 follow-up**;剩 flat-F4 / R1b / Q10 / phase15-D|
| 2026-05-18 | v1.25 | **Phase 3 R8 complete — PR3 orchestrator ship**(同 session R8 PR2 ship 后接 PR3,3/3 PR ship,Phase 3 进度 5/8,R8 R8-v1 done — R8-v2 enhancements queued)。commit `119bf12`(230 insertions / 2 files / 5 new PR3 orchestrator tests + 258 cumulative PASS / ~30 min vs plan 3.0d scope-reduced to pragmatic R8-v1)。**Scope ship**:`query_hierarchical(db, *, current_expression, hypothesis_pillar, region, universe, dataset_id, max_patterns=20, max_pitfalls=10, layer_budgets)` 主入口 sequential fall-through L0 → L1 → L2 → L3(NOT parallel union per [V1.0-A2-1] cost asymmetry + determinism + cache-friendly)+ `_consume()` closure 内嵌 dedupe by pattern_hash via seen_hashes Set,每 layer 增 layer_hits telemetry + 各 max budget cap(empty pattern_hash 仍 append 不 dedupe);每 layer skip guards(L0 需 current_expression / L1 需 expr OR pillar / L2+L3 需 expr);RAGResult dataclass 包 patterns + pitfalls + layer_hits + total_queries telemetry per plan §10 GO gate。**Tests**:5 PR3 orchestrator(empty no-inputs → 0 RAGResult / L0 exact hit short-circuit / dedupe across layers same pattern_hash 多 layer 命中 → 仅 1 / max_patterns 硬 cap 8 family matches → max 2 / Q9 decayed 0 surfaces SUCCESS across all 4 layers — plan §10 GO gate (d))。**R8-v2 follow-ups queued separate session**:(1) Redis cache per (layer, query_key) 5-min TTL + KB ingest invalidation;(2) R5 composite_score ranking — JOIN r1a_attribution_log on expression_hash for L2 SUCCESS re-ranking by mean(r5_composite_score) over recent rows (where r5_c1_aligned IS NOT NULL);(3) Dispatch in backend/agents/services/rag_service.py:RAGService.query reading settings.ENABLE_HIERARCHICAL_RAG;(4) flat-F3 PR2 wiring — backend/agents/llm_mutate_alpha.py default failure_context / decay_context 替换为 RAG#2 family output;(5) backend/tests/integration/test_rag_hierarchical_e2e.py 完整 8 E2E cases (flag-OFF byte-equiv + A/B PASS rate parity);(6) Production canary verify flag flip ON + 1 task 24h + per-layer hit-rate balanced (plan §10 GO gate b)。**Phase 3 进度 5/8 ship**(flat-F1+F2+F3+R9+R8 ✓);剩 flat-F4(1d cascade gated)+ R1b(4-6 周大改 Plan agent mode)+ Q10(5d pyqlib pre-screen)+ phase15-D(cascade 退役 + 4 周稳定 gated)|
| 2026-05-18 | v1.24 | **Phase 3 R8 PR2 ship — L1 pillar + L2 family layers**(同 session R8 PR1 ship 后接 PR2,2/3 PR complete,4/8 R8 layers ready,PR3 orchestrator 推下 session)。commit `d412c15`(356 insertions / 2 files / 11 new PR2 tests + 253 cumulative PASS / ~45 min vs plan 2.5d scope-focused)。**Scope**:add Layer 1 + Layer 2 to `backend/agents/hierarchical_rag.py` using PR1 backfilled meta_data JSONB keys。**Layer 1 pillar**:resolution priority hypothesis_pillar arg > infer_pillar(current_expression) fallback;pillar="other" short-circuit per [V1.0-A2-1]; SQL `WHERE meta_data @> '{"pillar_classified":<pillar>}'::jsonb` leverages PR1 GIN(jsonb_path_ops) index;SUCCESS 排除 decayed FAILURE 包括 decayed (plan §2.5 dual-filter);relevance_score=0.75。**Layer 2 family**:family_signature(current_expression)(operator-sequence sha256[:16] from R10 family_classifier),JOIN meta_data['family_signature']; **[V1.0-S5]** 排除 family_capped entries (`family_capped` truthy OR `_r10_family_cap_dropped`) — 否则 R10 purpose 被 undermined;空 signature ('<empty>' = bare field / no ops) → skip;relevance_score=0.65。R5 composite_score ranking 推 PR3 与 orchestrator 一起(避 r1a_attribution_log cross-table JOIN with different hash key 复杂度)。**Tests**:11 PR2 tests cover layer1(6 cases: empty/no-pillar / "other" short-circuit / explicit pillar match / decayed SUCCESS excluded / decayed FAILURE included / infer momentum from expression)+ layer2(5 cases: empty / no-ops / same-family-diff-fields match / family_capped excluded / decayed dual-filter)。**Default flag OFF zero behavior change**。**Phase 3 进度 4/8 ship + R8 PR1+PR2/3 → 5/8 in flight**;PR3 next session 3.0d(orchestrator query_hierarchical fall-through L0→L1→L2→L3 with total_cap=20 dedupe + Redis cache + R5 composite_score ranking + flag dispatch in RAGService.query + flat-F3 PR2 wiring + 8 integration tests + production verify)|
| 2026-05-18 | v1.23 | **Phase 3 R8 PR1 ship — hierarchical RAG schema + L0/L3 + GIN + backfill 3312 KB entries**(同 session R8 plan v1.0 ready 后接力 PR1 实施;plan 估 3.0d for PR1,实际 ~1.5h scope-focused)。commit `8b23702`(773 insertions / 6 files / 20 unit tests + 242 cumulative PASS)。**实施**:`backend/config.py` ENABLE_HIERARCHICAL_RAG default OFF + 7 RAG_HIER_* settings(budgets per-layer 5 / total cap 20 / cache TTL 300s / cross-region decay 0.7) + `backend/services/feature_flag_service.py` 双文件注册(25 SUPPORTED_FLAGS) + `backend/alembic/versions/b3c8d9e2f4a1` KB meta_data JSONB GIN index(jsonb_path_ops,applied production DB head `b3c8d9e2f4a1`,IF NOT EXISTS idempotent zero-risk additive) + **`backend/agents/hierarchical_rag.py`** (NEW ~245 行 pure-function module:DECAYED_KEY 集中常量 + `_FIELD_TOKEN_RE` + `_KNOWN_OPS` 集合 + `extract_fields_for_rag(expr)` 工具 + `RAGEntry` + `RAGResult` dataclasses + **`layer0_exact_match(db, *, current_expression, region, dataset_id, budget=5)`** pattern_hash UNIQUE lookup,SUCCESS 排除 decayed / FAILURE 包括 decayed Q9 dual-filter per [V1.0-S2],relevance_score=1.0 + **`layer3_field_level(db, *, current_expression, region, universe, budget=5)`** extract top-3 fields ILIKE OR,SUCCESS 排除 decayed + region filter (meta_data['regions'] List[str] OR ['region'] str OR missing→ANY per [V1.0-A1-3]),FAILURE 忽略 region,budget cap)+ `scripts/backfill_kb_pillar_family_signature.py` (NEW idempotent re-runnable script:for each KnowledgeEntry needing R8 keys (pillar_classified / family_signature / fields_used / pattern_operators_pending=True),compute via infer_pillar + family_signature + extract_fields_for_rag UPSERT meta_data,flip pattern_operators_pending=False) + `backend/tests/unit/test_rag_hierarchical_pr1.py` (NEW 20 tests pg_session:extract_fields 7 cases,RAGEntry/RAGResult defaults 2,layer0_exact_match 5 cases including decayed dual-filter,layer3_field_level 6 cases including budget cap + region filter)。**Production backfill 实证 LIVE**:`python scripts/backfill_kb_pillar_family_signature.py` → 3312/3312 KB entries backfilled,0 errors;`pillar_classified` + `family_signature` + `fields_used` populated + `pattern_operators_pending=false`。**Verification**:20/20 R8 PR1 PASS + 242/242 cumulative regression PASS。Default flag OFF zero behavior change(legacy RAGService.query unchanged,新 layer*_* helpers callable standalone for unit tests + 未来 orchestrator)。**PR2 next session 2.5d**:add layer1_pillar + layer2_family using backfilled meta_data JOIN + R5 composite_score from r1a_attribution_log for L2 ranking + +6 tests。**PR3 next+1 session 3.0d**:orchestrator(fall-through + dedupe + cache)+ query_hierarchical entry + flag dispatch in RAGService.query + flat-F3 PR2 wiring (failure_context + decay_context from RAG#2) + production verify + per-layer hit-rate telemetry。**Phase 3 进度 4/8 ship + R8 PR1/3 ship + R8 plan ready → 5/8 in flight**|
| 2026-05-18 | v1.22 | **Phase 3 R8 Hierarchical RAG plan v1.0 ready**(同 session R9 PR2 ship 后接力 R8 plan agent,plan agent + 3 轮 fresh adversarial review 模式同 R6 / R5 / flat-F1)。Plan `~/.claude/plans/phase3-r8-hierarchical-rag-2026-05-18.md` v1.0(931 行 / 62KB / §0-§16 完整 / 3 轮 review converged:self-audit 8+6 / fresh agent #1 5+4 / fresh agent #2 4+3 = **cumulative 17 MUST + 13 SHOULD inline-fixed,0 outstanding**)。**Scope**:替换 RAGService.query() 单层 SQL(dataset+pillar+region simple score-and-sort with V-24.C hallucination filter)为 **4-layer fall-through hierarchical retriever**(RAG#0 exact pattern_hash → RAG#1 pillar/theme infer_pillar JOIN → RAG#2 family_signature R10 复用 composite_score 排序 排除 family_capped per [V1.0-S5] → RAG#3 field-level current expr × dataset/region availability)。"additive overlay" pattern legacy code 不删,只加 query_hierarchical 新入口 + 内部 dispatch on flag。**Dual filter**:SUCCESS_PATTERN 严过滤 Q9 decayed (`COALESCE(meta_data->>'decayed','false') != 'true'`);FAILURE_PITFALL 反向命中 decayed (作 "avoid this decayed" hints)。**Redis cache per (layer, query_key) 5-min TTL** + KB ingest 时 invalidate。**Alembic GIN index** on KnowledgeEntry.meta_data + idempotent backfill script 给 3K+ existing entries 写 family_signature + pillar_classified。**flag** ENABLE_HIERARCHICAL_RAG default OFF 双文件注册。**Effort 8.5 人日** (vs §4.5 5-8d 基线 + 0.5d Redis cache + 0.5d flat-F3 deferred PR2 integration deliverable,[V1.0-S1] self-audit caught)。**3 PR 拆**:PR1 3.5d (flag + Alembic GIN + RAGResult schema + RAG#0 + RAG#3 + orchestrator skeleton + byte-equiv test) → PR2 3.0d (RAG#1 + RAG#2 + Redis cache + dedup) → PR3 2.0d (migration backfill + flat-F3 PR2 integration + GO gate metric scripts + production verify)。**GO gate**:(a) ≥5 production task flag ON ≥3 days (b) per-layer hit-rate balanced (RAG#0≥2% / #1≥15% / #2≥25% / #3≥30% over 200+ retrieves 防 single-layer 垄断) (c) 0 alpha PASS rate regression vs OFF baseline paired A/B 100 rounds allowed -2% variance (d) Q9 decayed 0 surfaces to LLM SQL audit (e) cascade T1 latency ≤ +50ms (cache absorbs) (f) flag OFF byte-equivalent integration test。**所有 R8 依赖 ready** (KB 3K+ / R5 composite_score / Q9 decayed marker / R10 family_signature + family_capped status / pillar_classifier / family_classifier)。**R8 ship 后 flat-F3 PR2 unblock** + R1b 实施时已有 KB-aware context。**实施推下 session**(本 session 30+ commits 已重,8.5d R8 impl 需 dedicated multi-session)。**Phase 3 进度 4/8 ship + R8 plan ready 5/8 → 待 impl**|
| 2026-05-18 | v1.21 | **Phase 3 R9 PR2 ship — node_simulate call-site swap + production verify**(同 session R9 PR1 ship 后接 PR2 wire-up,完成 R9 end-to-end production-active)。commit `34c7a6b`(44 insertions / 1 file / 116 regression PASS / 0 new test — PR2 是 integration not new logic)。**Scope**:wrap evaluation.py:node_simulate 两处 brain.simulate_batch 调用(L1080 smart-settings bucket loop + L1120 fallback else-branch)with cached_simulate_batch when ENABLE_SIMULATION_CACHE flag ON;soft-fall to direct BRAIN call on cache import OR DB error。Pattern:`if flag ON → try cached_simulate_batch(async with AsyncSessionLocal()) → except → fall back`;flag OFF byte-equivalent。**Production verify**:flag flipped ON via DB override + celery worker restart (PID 46137) + task 1538 (CHN FLAT, datasets=[pv1]) started — wait 10 min for first EVALUATE → first simulation_cache row;GO gate hit_rate ≥ 30% on duplicate-heavy workload 1-2 周后 verify。**24 SUPPORTED_FLAGS / 10 production ON** (R1a + DUAL_RAG + AST_DIM + DIR_BANDIT + FLAT_CONTINUOUS + TASK_SCHEMA_V2 + LLM_JUDGE + FAMILY_CAP + DAG_TRACE + **SIMULATION_CACHE**)。**Phase 3 进度 4/8** (R9 end-to-end ship);剩 flat-F4 / R1b / R8 / Q10 / phase15-D|
