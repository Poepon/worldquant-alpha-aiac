# 正交导向自动探索（Orthogonality-Steered Exploration）设计 plan **v5**

> v1 生成 2026-06-05 · v2 = 应用 3-镜头 fresh-agent review · v3 = 2-镜头收敛复审应用 MUST · v4 = orthogonality_score 指标重设计(shadow 暴露记 0 分)。
> **v5 = live verification 推翻 v4 的核心前提,简化为 1 行修复(2026-06-05)**。用真实 brain + 真实 sharpe-2.01 候选驱动**真正的 `_evaluate_single_alpha`**,观测到 `orthogonality_score=0.4936` 落库 ✅ —— 但 `_self_corr_source=**local**`,走的是**测量路径**,不是 v4 的 helper。读码坐实:**`get_with_fallback` 的 LOCAL 层 `calc_self_corr`(correlation_service.py:453-454)本来就 `_fetch_pnl_series(alpha_id)` 拉新候选 PnL + vs 池算 corr** —— 跟 v4 新增的 `compute_max_corr_vs_pool` 做的是同一件事。**v4 的「fresh 候选恒 UNKNOWN→需新 helper」前提是错的**(LOCAL 路径本就拉 fresh PnL;shadow 里的 UNKNOWN 真因是**刚 sim 完 PnL 未就绪 / 缓存失鲜**,helper 同样修不了)。**真正有效的只有 `:895` clobber 修复**(orthogonality_score 写进 `metrics` 局部、过 `{**metrics}` rebuild;原 ba65ac7 写 alpha.metrics 副本被覆盖、从未落库)。**v5 = 删冗余**:移除 `compute_max_corr_vs_pool` + UNKNOWN-fallback 分支 + `EVAL_ORTH_SHARPE_MIN` 门 + 冗余单测;orthogonality_score 逻辑回到 ba65ac7 + 写 `metrics` 局部(净改动 1 行)。flag 仍 gate **steering NUDGE**(node_hypothesis);metric 现 flag-无关(测到即记,控制臂也有值,利于 A/B)。**教训:重设计前先读既有降级路径 + live verify,别凭推断断言根因([[feedback_verify_root_cause_before_fix]])。**
...
> 状态:**Phase A 代码已建(foundation 正交 steering + orthogonality_score clobber 修复)+ live 验证(orthogonality_score=0.4936 经真 eval+真 brain 落库)。** steering NUDGE 由 flag gate;metric flag-无关。下一步:flag-ON 跑挖掘累积 orthogonality_score 分布 + 实测 SD 再进受控 A/B。注:idle-in-txn 泄漏(bandit SELECT 路径,a4edbb6 未修净)反复 wedge FLAT,是独立待修 infra bug。
...
- **orthogonality_score 接线(v5 简化,已建+live 验证)**:`node_evaluate` 的 `_evaluate_single_alpha`(`evaluation.py:~701`):`if self_corr_source != UNKNOWN: metrics["orthogonality_score"] = round(1 − self_corr, 4)`。self_corr 来自既有 `get_with_fallback`(其 LOCAL 层 `calc_self_corr` 本就拉 fresh 候选 PnL vs 池算 corr,覆盖新挖 alpha——只要 PnL 可取)。**关键:写 `metrics` 局部**(被 `:895` 的 `{**metrics,…}` spread 真持久化;原 ba65ac7 写 alpha.metrics 副本被该 rebuild 覆盖、从未落库 = clobber bug,这是唯一真修复)。flag-无关(测到即记,控制臂也有值);UNKNOWN→跳过。守卫单测 `test_node_orthogonality_score_fresh_candidate.py`(2:measured 过 rebuild 持久化 / UNKNOWN 不记)。**这是 dense 主指标的实现,先于一切 A/B。**
...
- **PRIMARY(shadow + A/B)**:`mean orthogonality_score` = `1 − self_corr_to_submitted_pool`(node_evaluate 经既有 `get_with_fallback`/`calc_self_corr` 拉候选 PnL vs 已提交池本地缓存算)。**v5 密度口径**:记于**所有进 self_corr 检查(`needs_corr_check`=meets_thresholds 或 score 过阈)且 self_corr 测得(非 UNKNOWN)的候选**——非 sharpe-pass-gated(那个门已删)。⚠ **真实覆盖瓶颈 = PnL-就绪**:刚 sim 完的候选 PnL 可能未就绪→UNKNOWN→无值(calc_self_corr 与已删 helper 都修不了)。若 live 累积过慢,考虑 deferred-retry(eval 后延迟重取 PnL 再回填)作未来增强。次辅:候选落 **0.4-0.7 正交带**的比例。SD 仍须 live 实测(plan §4 功率假设 SD~0.2 待证)。
