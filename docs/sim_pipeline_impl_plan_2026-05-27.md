...
> ⚠️ **前置依赖 / landmine(2026-05-27 shadow 期核实)**:pipeline 路径绕过 `mining_agent.run_evolution_loop`(直接调 `run(generate_only)` + sim/eval 子图),因此**只接了 RAG + 生成 + 持久化(含 alpha_failures + hypothesis_id 链接),没接 post-evaluate / round-end 的反馈"产出"侧**:R1b 重试回路、R1b hypothesis_mutate、G5 crossover 产出、B5 `_process_hypothesis_feedback` 归因(在 node_save_results 内,被 persister 替代而绕过)、dataset bandit reward 回填。**目前这些 flag(`ENABLE_R1B_*`/`ENABLE_G5_CROSSOVER`/`ENABLE_DATASET_VALUE_BANDIT`)全 OFF,legacy 也不跑 → 无回归、shadow 质量对比公平。但:启用这些 flag *之前* 必须先把它们在 pipeline 里接好(本节 F2 迁移),否则 flag 开了却在 pipeline 路径静默空转**(consume 侧已接但 produce 侧没有,正是 [[project_rdagent_costeer_loop_closure]] 那类空转开环陷阱)。RAG 主学习通道在 pipeline 里完整,不受影响。
...
- **F4 client-refresh**:`_refresh_brain_client` 从 per-round 改 per-T-seconds + in-flight 守卫(仅无 sim 在飞时 refresh,或每 consumer 独立 client)。✅ 已 ship `55b439e`(BrainClientRefresher drain-and-refresh barrier)。

## 自进化 / RD-Agent 机制 × pipeline 可行性映射(2026-05-27)

**判定原则**(pipeline 结构约束):`producer(生成)→队列→consumer(sim/eval,独立每候选)→队列→persister(单写者,看全流)`,**连续流无离散 round、stage 解耦、异步重叠**。
- 住**节点内** → 自动继承(producer/consumer/persister 复用节点)。
- **生成时输入** → producer 传 `config["configurable"]` 即可。
- **反馈类**(结果→再生成)→ 需 `consumer/persister→producer` 异步反馈通道;**连续流里比 legacy 的同轮 cycle 更自然**(反馈到了就再生成,不必等轮边界)。
- **要求整轮同步**(整轮结果齐了再决策)→ 与重叠流冲突,**必须改 窗口/累计/批次**。
- **真正"不能"的几乎没有**——差别是工作量 + 是否需重新概念化。

| 机制 | 可行性 | 怎么接 / 为什么 |
|---|---|---|
| RAG / KB 自进化 | ✅ 已应用 | node_rag_query 在 producer 生成图;persister 持续写 KB 读的 alpha/failure |
| RD-Agent 归因 shim `enhance_existing_node_evaluate` | ✅ 已应用(flag 开时) | 住 node_evaluate→consumer 跑=同 legacy(core 默认 DORMANT 3223 LOC) |
| Typed Hypothesis 谱系(hypothesis_id, level≥2) | ✅ 已应用 | persister 链 PASS+FAIL;per-candidate 持久化天然契合 |
| EvolutionStrategy + P2-C regime(静态) | ✅ 易接 | 生成时输入——producer 构 strategy 传 `run(config[strategy])`,~半天 |
| B5 归因 `_process_hypothesis_feedback`(CoSTEER attribution) | ✅ 可接 | 本质 per-alpha 非真 round-based → 移进 persister/consumer 逐候选调 |
| R1b retry(FAIL→重写→重 sim) | 🟡 反馈通道 | consumer/persister FAIL → 推 producer retry 队列 → 再生成;异步更自然 |
| R1b hypothesis mutate(失败假设→新假设) | 🟡 反馈通道 | 产出侧喂 producer(消费侧 `r1b_consumed_pending_hypothesis` 已接) |
| G5 轨迹交叉(2 PASS→offspring) | 🟡 反馈通道 | persister 累积 2 PASS → 触发 producer 交叉生成;事件驱动契合 |
| strategy 逐步进化(随结果调) | 🟡 连续适应 | 像 C-step2 按累计结果调,pipeline 已有这套(margin reward) |
| ROUND_SUMMARY | 🟠 重新概念化 | 连续流无 round → 改"批次/session 摘要"(F3) |
| round-history 早停 `should_stop_early` | 🟠 重新概念化 | round-based → 累计/窗口 = **R14 重设计**(已分析:硬上限 + bandit + Option C 多样性) |
| RD-Agent typed-pipeline ROUND 模式 | 🔴 大整合非移植 | 它本身是另一套解耦 DAG;stage 映射 producer(HypGen+Hyp2Exp)/consumer(ExpRunner)/persister(Exp2Feedback)→ **sim-pipeline ≈ 它的生产实现**,关系是合并/二选一 |

**核心洞见**:**sim-pipeline 本质就是 RD-Agent 解耦 pipeline 的生产落地**——`HypothesisGen→Hypothesis2Experiment→ExperimentRunner→Experiment2Feedback` 对应 `producer→(producer)→consumer→persister`。"把 RD-Agent 用进 pipeline"大半不是移植,而是**承认它们同源**,再把缺的反馈边(R1b/G5)按统一的异步反馈通道接上 → CoSTEER 闭环在 pipeline 的形态。

**→ F2 反馈通道规划**:R1b retry/mutate + G5 走**统一的 `consumer/persister → producer` 异步反馈队列**——producer loop 改为 `while: 先排空 feedback_queue(retry/offspring/mutated 候选)再按 Option C 选 dataset 生成`。反馈请求由 consumer(FAIL→retry)/persister(2 PASS→crossover、失败假设→mutate)推入。开 `ENABLE_R1B_*`/`ENABLE_G5_CROSSOVER` 前必须先接此通道(否则 produce 侧空转,见 [[project_rdagent_costeer_loop_closure]] 同类开环陷阱)。round-同步类(ROUND_SUMMARY/round 早停)单独走窗口/累计重构,不进反馈通道。
