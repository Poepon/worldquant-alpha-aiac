...
| **V-27.158 词汇表** | `calc_self_corr_by_window` | 阶段 C 统一了 `calc_self_corr` / `get_with_fallback` 为 `CorrSource`,但 `calc_self_corr_by_window` 的 per-window status(ok/insufficient_data/empty_pool/missing_window)未并入 — 是不同维度,有意保留 | 若日后要统一,需设计跨维度的 status 体系 |
