...
2. ~~**R1a attribution**~~ **【已决 → KEEP / 抽 enum,日志去留推迟 1c】**:`AttributionType` **必抽**(Phase 1a,断 core/integration.py:41 module import 让 core/ 可删),与日志无关。R1aAttributionLog **当前 live**(ENABLE_R1A_HOOK=TRUE / 5791 行 / 256 天 / 2 个无 fallback 读者:evolution_strategy.py:702 + hierarchical_rag raw-SQL)→ 保表 dormant,真正删/留**推迟 Phase 1c-delete**(届时有 flag-flip 数据)。
