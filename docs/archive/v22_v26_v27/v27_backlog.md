...
| 项 | File | 现象 | 根治方向 / 状态 |
|---|---|---|---|
| ~~**V-27.92 savepoint 粒度**~~ | `agents/graph/nodes/persistence.py` `_process_hypothesis_feedback` | 一轮多个 hid 共用**一个** `begin_nested()` savepoint —— 第 2 个 hid 的 `upsert_round_stats` 失败回滚,会连带回滚第 1 个已成功的 upsert,放大单 hid 失败的丢数据面 | ✅ 已闭环(2026-05-15,commit `6e2c689`)—— savepoint + try/except 移进 `for hid` 循环内,逐 hid 独立降级;34 个 V-27.92/B5-B6 测试通过 |
| ~~**V-27.92 flip-only 轮 attribution 隐身**~~ | `persistence.py` attribution + `should_abandon` | 一轮全是 flip 产物时 `real_alphas=[]`、`alpha_count=0`:V-27.68 guard 挡住 abandon、`mark_active` 因 count=0 不触发 —— hypothesis 实际产出了 flip alpha 却既不 active 也不计入 | ✅ 已闭环(2026-05-15,方案 1)—— ① `mark_active` 触发改 `real OR flip`;② flip-only 轮显式 `attribution="hypothesis"`(原方向证伪),不跑 LLM-on-empty;③ `should_abandon` 的 empty-round guard 改 `real==0 AND flip==0` 才算空轮(DB + memory 两路对称);保留 V-27.71 不 promote。53 个测试通过 |
...
savepoint 粒度 + flip-only attribution 已闭环。剩 V-27.1 claim 时序 ——
独立小修,优先级最低(触发需 config_snapshot 落库失败这一本就罕见的
前提),不专门排期、碰相关代码时顺带。

### 衍生增强 — 反向假设自动提出(flip-abandon → inverted hypothesis)

实现 flip-only attribution 修复时发现的高价值后续:一个 hypothesis 连续
flip-only(原方向持续证伪、符号翻转持续有效)现在会被正确 ABANDONED ——
但"这个因子有信号、只是方向相反"是极强的 KB 信号,目前丢失了。理想做法:
