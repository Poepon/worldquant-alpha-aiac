...
> 状态:**v4-final,3 轮对抗审查收敛(架构稳、9 执行漏洞已修),build-ready。Phase 0 已实施(2026-06-05):迁移 `p1d3f5a7c9e2`(hyp_intent + candidate_queue + alpha_failures.metrics)+ `config.POOL_*` 标定 + `test_pipeline_queue_schema.py`,Phase-0 三审全 GO、全 INERT 零读者、零回归。** LIVE DB 实证:6 反馈 flag 当前 ON / ENABLE_AUTO_ORCHESTRATOR OFF / CODE_GEN_SOFT_REG_W_ALIGNMENT=0.0。
...
- **Phase 0(地基,零行为变)✅ DONE 2026-06-05**:Alembic `p1d3f5a7c9e2` 建 candidate_queue + hyp_intent + alpha_failures.metrics;标定 p95-per-node token reservation(→ `config.POOL_*` + `docs/pool_token_calibration_2026-06-05.md`);定 experiment_runs 命运(选项 a 不动);committed schema 契约测。FLAT/ONESHOT 仍 live。
...
| 删 core/ 真 gate 是去 live R1a 调用点(`evaluation.py:3176`→integration→死簇),**非** AttributionType 抽取(后者只防 strand 类型,Phase 1a 已做)| 1b-flip 先关 ENABLE_R1A_HOOK,1c **同 commit** 删 R1a 块(evaluation.py:~3152-3290)+ integration.py + core/;顺序错/漏删 = 启动 import error |
...
## 6. 开工前必决(全部已决 2026-06-06)

1. ~~**experiment_runs 命运**(Phase 0)~~ **【已决 2026-06-05 → 选项 (a)】**:Phase 0 迁移 `p1d3f5a7c9e2` **不动** experiment_runs(只读 legacy 归档,保 runs.py)。新表锚 `hypotheses.id`、无 run_id FK;`alphas.run_id`/`trace_steps.run_id` 维持 nullable。run_id drop 推迟到 Phase 1d(runs.py 重指向 hyp_intent+candidate_queue 之后)。三审一致 GO。
2. ~~**R1a attribution**~~ **【已决 → KEEP / 抽 enum,日志去留推迟 1c】**:`AttributionType`+`HypothesisFeedback` **必抽**(Phase 1a,**为删 core/ 时不 strand 这两类型**,与日志无关)。**纠正(Phase 1a 审查)**:抽 enum **非**「使 core/ 可删」——`core/integration.py`(feedback import 在 :33,pipeline 在 :41)仍 module-import 整死簇;**core/ 可删的真 gate = 删 live R1a 调用点 `evaluation.py:3176`(1b-flip 先关 ENABLE_R1A_HOOK,1c 同 commit 删 R1a 块 + integration.py)**。R1aAttributionLog **当前 live**(ENABLE_R1A_HOOK=TRUE / 5791 行 / 256 天 / 2 个无 fallback 读者:evolution_strategy.py:702 + hierarchical_rag raw-SQL)→ 保表 dormant,真正删/留**推迟 Phase 1c-delete**(届时有 flag-flip 数据)。
3. ~~**ENABLE_ROBUSTNESS_CHECK**~~ **【已决 → 保 gate + 抽 helper + 删 GA 类】**:实测 **live TRUE**(非默认 OFF)——RobustnessGate 是 2026-06-03 上线的抗回测过拟合生产闸(evaluation.py:2670-2866,~10% PASS→PROVISIONAL,Bailey SR0 deflation)。保它 + 把它依赖的 `enumerate_window_perturbations`(genetic_optimizer.py:426-527 纯函数)抽到稳定模块(Phase 1a),**删 GeneticOptimizer 类(1405 LOC,从未实例化,仅测试引用)**(Phase 1c)。
4. ~~**supervisor 机制**~~ **【已决 → Python 父进程 Popen-respawn】**:新 `backend/pool_supervisor.py`(~50-100 LOC),跨平台单代码、读 `pool:{name}:drain` 优雅停(drain SET 则 park 不重启)、health 心跳防误重启、backoff/crash 计数。run.bat/run.sh 改启父进程。Windows 生产稳定后(1c soak)可换 NSSM。celery_app worker_process_init 的 brain:concurrent_sims 删须 guarded(终审 #5)。父进程死 = SPOF,需 init hook(Task Scheduler/run.bat)。
5. ~~**p95-per-node token reservation 标定**~~ **【已决 2026-06-05 → 已标定】**:实测 `llm_call_log`(11,301 调用/17 天)→ 落 `config.POOL_NODE_TOKEN_RESERVE`(code_gen 17000/hypothesis 14000/distill 4500/self_correct 5100/__default__ 5000,重节点取 p99)+ `POOL_TOKEN_BUDGET_PER_DAY=8M`(provisional runaway backstop)。全文:`docs/pool_token_calibration_2026-06-05.md`。POOL_* 非 ENABLE_ → 复标 = 改码 + 重启(非热翻)。
6. ~~**K_s/K_e 池 sizing**~~ **【已决 → K_s=2 / K_e=1 + 留 1 槽给 opt/auto-submit】**:live 角色=USER(3 槽,ENABLE_BRAIN_CONSULTANT_MODE 未设)。sim 是 63% 瓶颈→2 个 S worker 并行 sim,1 个 E(纯计算不占 BRAIN 槽),留 1 槽给 opt/auto-submit。Phase 1b 测 candidate_queue PENDING_SIM vs PENDING_EVAL 队列深度再调。CONSULTANT(80)升级用公式 K_s=min(slot/3,…) / K_e=max(2,slot−K_s−1)。**纠正:opt simulator 非 double-acquire**(simulator.py:136 每变体单 acquire/释放,旧 memory「2 槽/变体」错)。OPT_SIM_BUDGET_DAILY 软帽护挖矿(Phase 1b)。
7. ~~**〔终审 minor〕regime 块去留**~~ **【已决 → RECLAIM 删除】**:flag 从未在 DB 覆盖、live 遥测 30/12981=0.23% 纯测试噪声、唯一注入点 mining_agent.py:133-189 随 Phase 1c 删→regime 块(evaluation.py:1968-1990)变死码。Phase 1c 删 regime 块 + regime_classifier.py + 推断服务 + 3 regime 测;style preset 生成不依赖它仍独立存活。

## 6.5 Phase 0 已落地 + Phase 1b 契约(2026-06-05,三审 GO 后)

**Phase 0 已 ship**(迁移 `p1d3f5a7c9e2`,down_revision=`m4a9c7e2b1f8`):`hyp_intent` + `candidate_queue` 两表 + `alpha_failures.metrics` 列 + `config.POOL_*` 标定 + `backend/tests/unit/test_pipeline_queue_schema.py`(4 测,锁 schema 契约)。**全 INERT 零读者**(三审证伪)。`candidate_queue` 含 role-snapshot 一等列(`effective_default_test_period`/`effective_sharpe_submit_min`,终审 #7)。所有 FK `ondelete=SET NULL`。

**Phase 1b 必守契约(Phase 0 审查挖出,建池时遵循)**:
- **HG in-flight stage 字面量须 = `'CLAIMED'`**:`ix_hyp_intent_claim` partial WHERE `stage IN ('PENDING','CLAIMED')`。lease-recycle 扫描必须用 `stage='CLAIMED'`;若引入独立 `'GENERATING'` stage 则该 stage 不在 partial index 内 → seq-scan/漏行。建 stage 常量模块,index 与 loop 不漂。candidate_queue 同理 in-flight = `SIMULATING`/`EVALUATING`(已在 partial WHERE)。
- **POOL_* 直读 + 重启复标**:`settings.POOL_TOKEN_BUDGET_PER_DAY`/`POOL_NODE_TOKEN_RESERVE` 非 ENABLE_ 前缀 → 绕 `_flag_override_cache`,写 `FeatureFlagOverride` 静默 no-op([[reference_feature_flag_hook_enable_prefix_only]])。直读 settings;调参 = 改码 + 重启 worker。
- **task hard-delete**:FK 已全 `SET NULL`,故 `scripts/cleanup_historical_tasks.py` 不会被 queue 行 FK-block(plan 保 MiningTask 行 RETIRED 不删,故几乎不触发,但已防呆)。
- **stage `String(20)`**:现最长 `PENDING_EVAL`(12)。若 Phase 1b 加更长 stage(如 `PENDING_ROBUSTNESS`)同 revision 改 `String(32)`。
- **部署纪律**:`cd backend && alembic upgrade head` 必须先于任何写 `alpha_failures.metrics` 的 Phase 1b writer(`create_all` 不给既存表加列)。
- **Phase 1c 字节等价测**:hydrated 行 vs model_copy 整-state 的 run_evaluate PASS+FAIL byte-等价(§2)+ role-snapshot round-trip 契约测(终审 #7)。
