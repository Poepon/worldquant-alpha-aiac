...
@pytest.mark.asyncio
async def test_regime_current(client):
    with patch(
        "backend.services.regime_inference_service."
        "RegimeInferenceService.get_cached_regime",
        new=AsyncMock(return_value="calm"),
    ):
        r = await client.get("/api/v1/ops/regime/current?region=USA")
    assert r.status_code == 200
    assert r.json() == {"region": "USA", "regime": "calm", "source": "service"}


@pytest.mark.asyncio
async def test_regime_snapshot_from_redis(client, monkeypatch):
    fake = MagicMock()
    fake.get.return_value = json.dumps({
        "regime": "elevated", "confidence": 0.8,
    }).encode("utf-8")
    monkeypatch.setattr("backend.tasks.redis_pool.get_redis_client", lambda: fake)
    r = await client.get("/api/v1/ops/regime/snapshot?region=USA")
    assert r.status_code == 200
    body = r.json()
    assert body["source"] == "service"
    assert body["snapshot"]["regime"] == "elevated"


@pytest.mark.asyncio
async def test_regime_snapshot_fallback_to_archive(client, docs_root, monkeypatch):
    fake = MagicMock()
    ...
    assert body["snapshot"]["regime"] == "calm"


@pytest.mark.asyncio
async def test_regime_history(client, docs_root):
    today = OpsReportReader.today_sh()
    _write(docs_root, "regime_state", today, {
        "regions": {"USA": {"regime": "calm", "pass_rate": 0.6}},
    })
    r = await client.get("/api/v1/ops/regime/history?region=USA&days=5")
    assert r.status_code == 200
    body = r.json()
    assert len(body) == 1
    assert body[0]["regime"] == "calm"


@pytest.mark.asyncio
async def test_regime_rerun(client):
    fake_redis = _MiniRedis()
    with patch("backend.celery_app.celery_app") as mock_app, \
         patch("backend.services.ops_service.OpsService._redis", return_value=fake_redis):
        mock_app.send_task.side_effect = lambda name, **k: _AsyncResult("regime-id")
        r = await client.post("/api/v1/ops/regime/rerun?region=USA")
    assert r.status_code == 200
    assert r.json()["name"] == "backend.tasks.run_regime_infer"
