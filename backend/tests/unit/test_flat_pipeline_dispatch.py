"""FLAT mining helper tests (post serial→pipeline migration Phase C, 2026-05-29).

History: this module originally tested ENABLE_SIM_PIPELINE flag dispatch (flag
ON → pipeline / OFF → legacy serial). After Phase C the legacy serial path was
retired — `_run_flat_iteration` IS the pipeline — so the dispatch tests were
removed. Remaining tests cover the helper functions still relied on by the
pipeline (picker, op_timeout, task-skip guard, redelivery guard).
"""
...
async def test_pipeline_next_round_inputs_smoke(monkeypatch):
    """Run _run_flat_iteration far enough to execute next_round_inputs
