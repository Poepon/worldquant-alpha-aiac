...
# Window-perturbation primitives moved to the pure leaf module
# backend/window_perturbation.py (Phase 1a, four-pool decoupling). Re-imported
# here so the GA class's mutate_window_parameter + legacy test importers resolve
# until Phase 1c deletes this dead GeneticOptimizer file. The LIVE RobustnessGate
# path imports them directly from backend.window_perturbation, not from here.
from backend.window_perturbation import (  # noqa: F401  (re-export for back-compat)
    WINDOW_VALUES,
    _find_window_sites,
    enumerate_window_perturbations,
)
...
# WINDOW_VALUES moved to backend/window_perturbation.py (Phase 1a), re-imported above.
...
# _WINDOW_NAME_RE / _INT_LITERAL_RE / _find_window_sites / enumerate_window_perturbations
# were extracted to the pure leaf module backend/window_perturbation.py (Phase 1a,
# four-pool decoupling) and are re-imported at the top of this module for backward
# compat. See that module for the implementations. mutate_window_parameter above
# (a dead GA-class helper, deleted with the class in Phase 1c) still references the
# re-imported _find_window_sites + WINDOW_VALUES.
