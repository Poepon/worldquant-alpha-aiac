...
async def node_evaluate(
    ...
    # Backfills 2 of the R12-sentinel-tied stamps onto alpha.metrics so the
    # R12 decision counterfactual SQL can attribute per-sentinel PASS rate
    # margin. Sources of truth already exist:
    ...
    # (R1b mutation stamp retired 2026-06-13 with the R1b loop.)
    ...
    if updated_alphas:
        try:
            ...
            if any(_stamp_count.values()):
                logger.info(
                    "[PR0.6 stamp] forest=%d cache=%d / total=%d",
                    _stamp_count["forest"],
