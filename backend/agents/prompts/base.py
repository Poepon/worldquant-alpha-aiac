...
def build_strategy_constraints(ctx: PromptContext) -> str:
    ...
    constraints.append(
        ...
        "is one of {market, sector, industry, subindustry} (or a GROUP-typed "
        "field from the provided field list). "
        "The grouping argument of ANY group_* operator (group_neutralize / "
        "group_mean / group_zscore / ...) MUST be one of those group keywords "
        "or a GROUP-typed field — it is NEVER the universe name (e.g. top3000, "
        "top1000), a regular data field, or a number. Those are not groups and "
        "BRAIN rejects them ('Incompatible unit ... expected Unit[Group:1]'). "
