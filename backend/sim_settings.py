...
truncation per alpha based on expression form + field metadata.

Why this exists:
- Default settings (delay=1, decay=4, neutralization=SUBINDUSTRY, truncation=0.08)
  are reasonable for raw ts_op signals.
- Wrappers like `group_neutralize(X, industry)` already neutralize at the
  expression level. Simulating with BRAIN's neutralization=SUBINDUSTRY on top
  would double-neutralize and erode signal.
- `trade_when(filter, X, exit)` changes turnover dramatically; the default
  decay=4 fights the entry-filter logic.
- Slow fundamental fields benefit from higher decay (16-32) to keep turnover
  in the [0.01, 0.55] band; fast price-volume fields fail with decay > 2.
...
Public API:
    smart_simulation_settings(expression, *, region="USA", universe="TOP3000",
                              test_period=None, field_category=None,
                              overrides=None) -> Dict
    settings_reason(expression, field_category=None) -> str

The reason string is meant for telemetry / metadata.sim_settings_reason so
post-mortem analytics can correlate settings choices with PASS rates.
"""
from __future__ import annotations

from typing import Dict, Optional

from backend.alpha_expression_utils import (
    ...
    *,
    ...
    if parsed:
        ...
        if op in _INTRINSIC_NEUT_OPS:
            ...
            s["decay"] = 0
            # trade_when with intrinsic-neut inner — both rules apply
            args = parsed[1]
            if len(args) >= 2:
                inner = args[1].strip()
                inner_naked = _is_negation_wrapper(inner) or inner
