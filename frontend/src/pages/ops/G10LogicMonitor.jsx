...
export default function G10LogicMonitor() {
  ...
  if (error) {
    return (
      <Alert type="error" showIcon message="加载 G10 逻辑库失败"
        description={error?.response?.data?.detail || error?.message} />
    )
  }
  if (!data) return <Empty description="无 G10 数据" />
  ...
  return (
    <div>
      ...
      <Alert
        ...
        message={
          <Space wrap>
            <span>逻辑蒸馏开关 (ENABLE_G10_LOGIC_DISTILL)：</span>
            <Tag color={flagOn ? 'green' : 'default'}>{flagOn ? '已开启' : '已关闭'}</Tag>
            <Text type="secondary">每周日 03:00 SH 蒸馏 · 注入需另开 ENABLE_G10_LOGIC_INJECT</Text>
          </Space>
        }
      />
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 16 }}>
        ...
        <Col xs={12} sm={6}>
          <Card className="glass-card">
            <Statistic title="窗口蒸馏成本" value={data.weekly_total_cost_usd ?? 0}
      ...
      <Card className="glass-card" title={`活跃逻辑条目（前 ${data.entries?.length ?? 0}）`}>
        <Table
          ...
          columns={[
            ...
            { title: '支柱', dataIndex: 'pillar', width: 110,
            ...
            { title: '源 alpha 数', dataIndex: 'source_alpha_count', width: 100, align: 'right' },
            { title: 'vs 上周相似', dataIndex: 'similarity_jaccard_to_prev_week', width: 110, align: 'right',
