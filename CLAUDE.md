...
**Retired in `b89b732` (2026-06-06 — do NOT look for these; this file's pre-06-06 version described them as live):** `tasks/mining_tasks.py` / `run_mining_task` / `_run_flat_iteration` / `run_flat_pipeline_session`; FLAT / ONESHOT / CASCADE schedules + `POST /api/v1/tasks` + `/ops/start-flat-session` + `/ops/flat-sessions/*` endpoints; `tasks/orchestrator.py` (`ENABLE_AUTO_ORCHESTRATOR`); `mining_agent.py` / `strategy_agent.py` / `evolution_strategy.py` / `field_screener.py` / `feedback_r1b.py` / `feedback_g5.py` / `genetic_optimizer.py`; `agents/core/` source (only `__pycache__` remains; `AttributionType` lives in `agents/attribution_types.py`); `agents/knowledge_extraction.py`; `experiment_runs` table. (RAG read path `agents/services/rag_service.py` + `agents/hierarchical_rag.py` SURVIVE — pool HG stage uses them.) DB still has 71 historical FLAT/ONESHOT `MiningTask` rows (query-only, can't restart). `feedback_agent.py` survives (consultant/sync path). **Phase 2 (async cognitive reconcile, `ENABLE_POOL_COGNITIVE_RECONCILE`) NOT yet activated** — feedback is the pool E-stage synchronous write, not yet an async reconcile beat.

**Retired 2026-06-13 (`87d3783` — 精减代码, −3.5k LOC; do NOT look for these):** the **R1b CoSTEER retry/mutate loop** (already unreachable after the FLAT retirement; its flags were set false 2026-06-06): `agents/graph/nodes/r1b_loop.py` / `r1b_persistence.py`, `agents/prompts/r1b_{retry,mutate}.py`, `tasks/failure_tree_pruner.py`, `models/r1b_retry.py` (`R1bRetryLog`) + all `ENABLE_R1B_*` / `R1B_*` settings + the 5 `R1b-CoSTEER` FlagSpecs + the ops endpoints `/ops/r1b/telemetry` & `/ops/r1b/chain-depth-distribution` + the orphan `/ops/costeer/deploy-recommendation` + `knowledge_extraction.record_failure_tree` + `hierarchical_rag._elevate_failure_tree_pitfalls` + `evaluation._pr06_lookup_mutated_hypothesis_ids` (and its `_r1b_mutation_triggered` stamp; the G8-forest + R9-cache stamps in that block SURVIVE). **DB dropped** (migration `e9c1a7f3b5d2`): the `r1b_retry_log` table + `hypotheses.parent_hypothesis_id` / `r1b_mutation_depth` columns + the `Hypothesis.parent_hypothesis` self-relationship. Also retired: the dead LLM-routing nodes **`agents/llm_mutate_alpha.py`** (sole caller `tier_seed` T2 deleted in `0350c20`) and the **`agents/graph/attribution.py`** LLM classifier (`_process_hypothesis_feedback` now uses the heuristic `early_stop.classify_attribution`) — and the `LLM_FUNCTION_MODEL_MAP` default + live override trimmed to the 4 live node_keys (`hypothesis` / `code_gen` / `self_correct` / `distill_context`) + `__default__` (the dead `r1b_retry` / `r1b_mutate` / `llm_mutate_alpha` / `attribution` / `llm_crossover_alpha` / `r5_alignment_c1` / `r5_alignment_c2` keys removed). And the **orphan `MiningWorkflow` methods** `run` / `run_with_persistence` / `create_mining_graph` / `_build_graph` / `_build_generation_graph` (the legacy full-graph + FLAT-batch-persist path; the pool only calls `run_hypothesis` / `run_codegen` / `run_simulate` / `run_evaluate` + the G5 `run_validate`) — `workflow.py` shrank 1207→197 lines. `feedback_agent.py` (failure_analysis/round_analysis) + the G5 crossover path SURVIVE.
